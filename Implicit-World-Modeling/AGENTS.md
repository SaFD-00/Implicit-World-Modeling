...
### 모델 추가
1. 노트북 Cell 5 `_MODEL_CONFIG` 에 모델 항목 추가 (필드: `model_id`, `short_name`, `template`, `size` (`"7-9B"` / `"3-4B"` 2 단 tier), image-pixel 은 `_img_cfg(short)` 헬퍼로 family config 에서 자동 주입 — 같은 셀의 `MODEL_FAMILY_CONFIG` 에 먼저 등록하지 않으면 `_img_cfg` 가 `KeyError` 를 낸다).
...
- merge 순서 (단일 진실원: 노트북 Cell 5 CONFIGS 빌더): `_DATASET_CONFIG` baseline → `_SIZE_CONFIG_AC[size]` (AC_EXP01 ~ AC_EXP05 일 때만 — MC 는 미적용) → `hparam_overrides`. 전체 표는 [`ARCHITECTURE.md`](./ARCHITECTURE.md) §2.
