...
- **stage1-only 가드**: 노트북 Cell 5 의 `_DATASET_CONFIG` (+ `_STAGE1_ONLY = {"MonkeyCollection", "AndroidControl_EXP04"}`, `_DUAL_TASK_TEST` 에 AC_EXP04 포함, cutoff 24576 분기, `_pd_ds` per-device batch 분기) 에 모두 등록한다. `filter_long_samples.py` · `eval_viewer.py` 도 AC_EXP04 를 stage1-only 로 인식. dataset_info 는 **stage1 5 키만** 등록한다.
- **YAML 은 Cell 10 으로 재생성하지 말 것**: EXP04 stage1 YAML 은 **EXP03 stage1 YAML 을 복사** 해서 만든다. 노트북 Cell 10 은 EXP03 의 single-H100 hand-fix 를 덮어쓰고 git-ignored 라 복구 불가하다. GPU 0,1 (NPROC=2) 에서 global batch 64 를 유지하려면 lora `gradient_accumulation_steps=32` 로 baked. 실제 학습은 두 모델 (qwen3-vl-8b → qwen2.5-vl-7b) stage1 LORA, tmux [`scripts/tmux_exp04_stage1.sh`](./scripts/tmux_exp04_stage1.sh).
