...
- **3 개 Vision-Language 모델**: `Qwen/Qwen3-VL-8B-Instruct` (`template=qwen3_vl_nothink`, 7-9B tier), `Qwen/Qwen2.5-VL-7B-Instruct` (`template=qwen2_vl`, factor 28 → image budget 1,605,632, 7-9B tier), `Qwen/Qwen2.5-VL-3B-Instruct` (`template=qwen2_vl`, factor 28 → image budget 1,605,632, **3-4B tier**, EXP05 용 신규). **native 좌표 규약이 서로 다름** — Qwen3-VL 은 grounding 좌표를 **0–1000 정규화** (해상도 독립) 로, Qwen2.5-VL 은 입력 이미지 실제 해상도 기준 **절대 픽셀** 로 출력한다 (Qwen2-VL 의 0–1000 정규화 → Qwen2.5-VL 절대 픽셀 → Qwen3-VL 다시 0–1000 정규화 로 세대별 반전). 우리 좌표 실험군 (EXP03/EXP04) 데이터는 **0–1000 정규화** 로 통일 (root div `[0,0][1000,1000]`) → Qwen3-VL native 와 일치, Qwen2.5-VL 은 native (픽셀) 와 어긋나 mismatch. **이 mismatch 때문에 EXP03/EXP04 는 Qwen3-VL 전용 실험군이다** (`qwen3-vl-8b` 만 학습·평가; Qwen2.5-VL 은 사용하지 않는다). **대칭으로 EXP05 는 절대 픽셀 좌표 (840×1876, base 1080×2400, budget 1,605,632, factor 28, min 3,136) 라 Qwen2.5-VL native 와 일치 → Qwen2.5-VL 전용 실험군이다** (`qwen2.5-vl-3b` / `qwen2.5-vl-7b` 만 학습·평가; `qwen3-vl-8b` 는 factor 32 + 0–1000 정규화 native 로 좌표 정합이 원리적으로 불가해 제외). 상세는 "좌표 표현 실험군 좌표계 규약" 서브섹션. (사용자 요청의 "Qwen2.5-VL-3B/8B 모두" 에서 **8B 는 `qwen2.5-vl-7b` 로 해석** — Qwen2.5-VL family 에 8B 체크포인트가 없고, 유일한 8B 인 `qwen3-vl-8b` 는 factor·좌표 규약 이중 mismatch 로 EXP05 부적합.)
- **3 학습 데이터셋 + 1 평가 전용 벤치마크**: AC_EXP01, AC_EXP02, MC 가 학습 대상, MB 는 평가 전용. AC (AndroidControl) 자체는 학습/평가 entry 가 아니라 AC_EXP01/AC_EXP02 의 **원본 source 자산** (이미지 + 원본 jsonl + `episodes_meta.jsonl`) 으로만 보존된다. AC_EXP01 는 Stage 1 ratio mix (state_pred:action_pred = 3:7, 5:5, 7:3) 3 종을 sweep 해 별개의 가중치를 산출하고, **Stage 2 도 같은 ratio sweep 으로 활성** — Stage 1 ratio merged 를 base 로 같은 stage2 데이터 (`implicit-world-modeling_stage2_{train,test_id,test_ood}.jsonl`) 를 학습. stage2 데이터 자체는 ratio 와 무관 (3 ratio 공유) 이며, ratio 차원은 **stage1 → stage2 base 계보** 로만 흐른다. `_STAGE1_ONLY = {"MonkeyCollection", "AndroidControl_EXP04", "AndroidControl_EXP05"}` 라 AC_EXP01 는 Stage 1/2 모두 파이프라인에 참여하고 MC · **AC_EXP04** · **AC_EXP05** 만 Stage 1 전용이다. **AC_EXP04** (`AndroidControl_EXP04`) 는 좌표(point) 표현 실험군 **AC_EXP03** (AC_EXP01 ratio73 멤버십의 좌표 미러) 의 **stage1 프롬프트 업그레이드** 변형으로, EXP03 과 동일 `(episode, step)` 멤버십·좌표 표현을 유지하고 action space `scroll(direction,point)`→`swipe(start,end)`, role 문구 "represented as html-style XML", `[SWIPE]` 규칙만 다르다. **Stage 1 전용 dual-task** (state+action) 로만 학습·평가하고 **Stage 2 는 보류** — dataset_info stage1 5 키만 등록, stage2 YAML/등록/eval 은 없다. **AC_EXP05** (`AndroidControl_EXP05`) 는 **AndroidWorld 해상도 정렬 실험군** 으로, AC_EXP01 ratio73 멤버십을 **절대 픽셀 좌표(840×1876)** 미러 (`scripts/mirror_exp05.py`) 한 것이다 — 소스 2 파일은 `data/AndroidControl/implicit-world-modeling_stage1_{action,state}_xy_pixel-aligned.jsonl` (Google Drive '0710_버젼' 의 raw `stage1_0710_{action,state}_pred.jsonl` 을 이 canonical 이름으로 rename; 출력 이미지 경로는 mirror 가 EXP01 의 `AndroidControl/images/...` 재사용). 출력 stage1 7 파일 총 **64,787 행** (train **47,556** / test 각 ~2,854–2,885, drop 2,444). **Qwen2.5-VL 전용**, Stage 1 전용 dual-task, cutoff 24576, half-batch, **Stage 2 보류** (EXP04 와 동일 — stage1 5 키만 등록).
...
좌표(point) 실험군 EXP03·EXP04 의 XML 은 노드마다 `bounds="[x1,y1][x2,y2]"` (바운딩 박스) + `point="[x,y]"` (중심점) 를 갖고, action 의 `point` 는 해당 노드의 `point` 속성을 **verbatim 복사** 하도록 시스템 프롬프트가 강제한다 (`[POINT]` / `[NO HALLUCINATION]`). 좌표값은 **0–1000 정규화** — root div 가 `[0,0][1000,1000]` (`point="[500,500]"`) 이고 전 좌표 min 0 / max 1000. 원천은 `data/AndroidControl/implicit-world-modeling_stage1_{action,state}_xy.jsonl` (EXP03) 및 `*_xy_prompt-enhanced.jsonl` (EXP04) — 둘 다 0–1000 정규화. **픽셀 좌표가 아님.**

- **Qwen3-VL 전용 실험군**: 이 0–1000 정규화는 **Qwen3-VL native 와 일치** 하지만 **Qwen2.5-VL native (절대 픽셀) 와는 어긋난다**. 이 좌표계 mismatch 때문에 **EXP03/EXP04 는 `qwen3-vl-8b` 만 학습·평가하는 Qwen3-VL 전용 실험군** 이다 — Qwen2.5-VL 은 정규화 좌표를 native 로 쓰지 않아 (자기 pretrain 규약과 충돌) 이 실험군에 넣지 않는다. 좌표 표현(index→point) 효과는 **동일 Qwen3-VL 계열 내 EXP01 ratio73 대조군** 과 비교하고, 픽셀 미러는 두지 않는다. (EXP04 tmux 스케줄·eval_viewer 등에서 `qwen2.5-vl-7b` 를 EXP03/EXP04 대상으로 넣지 말 것.)
- 근거: Qwen3-VL 은 정규화→픽셀 rescale 시 분모로 **999** 사용 (1000 아님, backward-compat). 출처 — [Qwen3-VL Spatial Understanding & 2D Grounding (DeepWiki)](https://deepwiki.com/QwenLM/Qwen3-VL/5.2-spatial-understanding-and-2d-grounding), [Qwen2.5-VL Visual Grounding issue #866 (QwenLM)](https://github.com/QwenLM/Qwen2.5-VL/issues/866).
- **EXP05 는 정반대 규약 (절대 픽셀, Qwen2.5-VL 전용)**: EXP05 좌표는 **절대 픽셀 840×1876** (base 1080×2400 을 budget 1,605,632 / factor 28 로 smart_resize 한 이미지의 실제 픽셀; min 3,136) 이라 **Qwen2.5-VL native 와 일치**, EXP03/EXP04 의 0–1000 정규화와는 규약이 다르다. 따라서 **EXP05 는 `qwen2.5-vl-3b`/`qwen2.5-vl-7b` 만** 학습·평가하고 `qwen3-vl-8b` 는 넣지 않는다 (아래 "EXP05 실험군 작업 시").
...
- **데이터 산출**: [`scripts/mirror_exp04.py`](./scripts/mirror_exp04.py) 가 source `data/AndroidControl/implicit-world-modeling_stage1_{action,state}_xy_prompt-enhanced.jsonl` 을 EXP01 ratio73 멤버십으로 미러해 `data/AndroidControl_EXP04/` 에 stage1 train (49,276) + test 6 종 (총 7 파일) 을 만든다. `split_data.py` 가 아니라 mirror 스크립트가 기준 (EXP03 와 동일 패턴).
...
- **stage1-only 가드**: 노트북 Cell 5 의 `_DATASET_CONFIG` (+ `_STAGE1_ONLY = {"MonkeyCollection", "AndroidControl_EXP04", "AndroidControl_EXP05"}`, `_DUAL_TASK_TEST` 에 AC_EXP04 포함, cutoff 24576 분기, `_pd_ds` per-device batch 분기) 에 모두 등록한다. `filter_long_samples.py` · `eval_viewer.py` 도 AC_EXP04 를 stage1-only 로 인식. dataset_info 는 **stage1 5 키만** 등록한다.
- **YAML 은 Cell 10 으로 재생성하지 말 것**: EXP04 stage1 YAML 은 **EXP03 stage1 YAML 을 복사** 해서 만든다. 노트북 Cell 10 은 EXP03 의 single-H100 hand-fix 를 덮어쓰고 git-ignored 라 복구 불가하다. GPU 0,1 (NPROC=2) 에서 global batch 64 를 유지하려면 lora `gradient_accumulation_steps=32` 로 baked. 실제 학습은 **`qwen3-vl-8b` 단일 모델 (Qwen3-VL 전용 실험군)** stage1 LORA, tmux [`scripts/tmux_exp04_stage1.sh`](./scripts/tmux_exp04_stage1.sh) (좌표계 mismatch 로 `qwen2.5-vl-7b` 는 제외).
...
- **git-ignored**: `data/`, `outputs/`, `LlamaFactory/` 전체가 `.gitignore` 대상이라 EXP04 데이터·YAML·`dataset_info.json` 은 로컬 전용이다 (커밋되지 않음).

### EXP05 실험군 (AndroidWorld 해상도 정렬, 절대 픽셀, Qwen2.5-VL 전용, Stage 1 전용) 작업 시
AC_EXP05 는 AC_EXP01 ratio73 멤버십을 **절대 픽셀 좌표(840×1876)** 로 미러한 AndroidWorld 해상도 정렬 실험군이다 (base 1080×2400, `--image-budget 1605632` → 좌표 840×1876, factor 28, min 3,136). EXP03/EXP04 의 대칭 (정규화↔픽셀) 이며 **Qwen2.5-VL 전용** (`qwen2.5-vl-3b`/`qwen2.5-vl-7b`).

- **데이터 산출**: [`scripts/mirror_exp05.py`](./scripts/mirror_exp05.py) 가 source `data/AndroidControl/implicit-world-modeling_stage1_{action,state}_xy_pixel-aligned.jsonl` 을 EXP01 ratio73 멤버십으로 미러해 `data/AndroidControl_EXP05/` 에 stage1 train (**47,556**) + test 6 종 (총 7 파일, **64,787** 행) 을 만든다. 소스는 Google Drive '0710_버젼' 의 raw `stage1_0710_{action,state}_pred.jsonl` 을 위 canonical 이름으로 rename 한 것 (mirror 미도착 시 traceback 없이 안내 후 exit 1). 출력 이미지 경로는 EXP01 의 `AndroidControl/images/...` 재사용 (source 의 `myset/images/...` 는 매칭 키 추출용).
...
- **diff loss v2 필수 (EXP05)**: Stage 1 학습에 `use_diff_token_weighted_loss: true` 를 적용하되, AC_EXP02 가 쓰는 v1 (`scripts/diff_loss/{hungarian_metric,hungarian_diff,token_weight_builder,preprocess_dataset}.py`) 이 아니라 **v2** (동일 이름 + `_v2.py`) 를 써야 한다 — v1 파일은 그대로 두고 v2 4파일을 병존시킨다 (EXP02 재현성 보존). **EXP05 에 v2 가 필수인 이유**: EXP05 HTML 에는 `index` 속성이 아예 없다 (실측: index 0개, bounds 48개). v1 builder 는 `index="..."` 를 regex 로 필수 요구하므로 EXP05 에 v1 을 쓰면 모든 토큰이 baseline 으로 방치되어 diff loss 가 조용히 무력화된다. v2 는 위치 cost 를 DOM index 대신 **bounds 중심점 거리** 로 교체 (`W_POS=0.4`, `BOUNDS_NORM=2050.0`=840×1876 대각선, `BOUNDS_TAU=50.0`), `_collect_texts()` 가 자손 텍스트 흡수를 제거하고 direct text + 자체 속성(description/id/text/aria-label)만 사용, `MATCH_THRESHOLD` 1.5→1.7, element 키가 index→bounds 폴백으로 확장, metric key 가 `hungarian_idx`→`hungarian_pos` 로 바뀐다.
- **신규 diff loss 가중 체계**: 기존 AC_EXP02 는 diff 토큰 2.0배 / non-diff 1.0배였으나, EXP05 는 Qwen Agent World 방식으로 **diff 토큰 1.0배 / non-diff 토큰 0.25배** 를 쓴다. intermediate action 예측 샘플은 diff 계산을 건너뛰고 **uniform 1.0** 을 준다 (샘플 판별은 `images` 개수: 1개=state_pred, 2개=action_pred). 배수 자체는 줄었지만 non-diff 도 함께 낮아져 diff 가 non-diff 대비 실질 **4 배** 강해진다 (근거: 2026-07-11 Slack DM + Google Meet 회의록).
...
- **YAML/실측**: EXP05 stage1 YAML 6 개 전부 `use_diff_token_weighted_loss: true` (노트북 Cell 5 가 SSoT). `token_weights` 는 train jsonl 인라인 필드라 `dataset_info.json` 컬럼 등록은 불필요 (`converter.py:226` 이 raw jsonl 에서 직접 읽음). train 실측 47,556 행 = state 33,285 + action 14,271, weight 값 분포는 state `{0.25, 1.0}` / action `{1.0}`.
...
- **노트북 Cell 5**: `_MODEL_CONFIG[qwen2.5-vl-3b]` (size `3-4B`), `_PER_DEVICE_BS_BY_SIZE["3-4B"]`, `_DATASET_CONFIG[AndroidControl_EXP05]`, `_DUAL_TASK_TEST`·`_STAGE1_ONLY` 등록, cutoff 24576·half-batch tuple 에 EXP05 포함. image budget 은 family 기본 (1,605,632/3,136) 이라 `image_overrides` 불필요.
- **Cell 10 은 allowlist 로 EXP05 만 생성**: `_YAML_GEN_DS = {"AndroidControl_EXP05"}` — EXP03/04 hand-fix YAML 재생성 차단 (전체 재생성 필요 시 None 으로, 단 EXP03/04 백업 선행). **Cell 14 는 파일 부재 dataset 을 raise 대신 `[skip]` 완화** (EXP04 로컬 부재로도 셀 완주).
- **Stage 2 는 보류** (EXP04 와 동일 — `stage2_eval.sh` 미추가, stage1 5 키만 등록).
- **git-ignored**: EXP05 데이터·YAML·`dataset_info.json` 은 로컬 전용.
- **모델 해석**: 사용자의 "3B/8B 모두" 에서 8B = `qwen2.5-vl-7b` (Qwen2.5-VL 에 8B 없음; `qwen3-vl-8b` 는 좌표·factor 이중 mismatch 로 제외).
