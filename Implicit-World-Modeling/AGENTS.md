...
- **데이터셋 역할 분리**:
  - 학습 대상 DS: `AndroidControl` (AC), `AndroidControl_EXP01` (AC_EXP01), `AndroidControl_EXP02` (AC_EXP02), `MonkeyCollection` (MC).
  ...
  - 평가 스크립트는 `--train-dataset {AC|AC_EXP01|AC_EXP02|MC}` + `--eval-datasets AC,AC_EXP01,AC_EXP02,MC,MB` 로 학습 DS 와 평가 DS 를 분리. Stage 2 eval 은 `AC | AC_EXP01 | AC_EXP02` (MC 미지원 — 데이터 없음). AC_EXP01 는 `--exp01-ratio` 로 학습 모델 ratio 를 단일 지정.
...
### 데이터 분할 규칙
- [`scripts/split_data.py`](./scripts/split_data.py) 가 기준. AC 는 Stage 1 / Stage 2 모두 app-level ID/OOD (단일 partition 공유), MC 는 Stage 1 random split (메타 없음, 자동 fallback), MB 는 split 없음.
...
- 흐름: **`train → merge → eval`** — `stage1_merge.sh` 가 모든 epoch 를 각각 local merge + HF push (`trainer_state.json.epoch` 파싱), `stage1_eval.sh` 가 `--train-dataset {AC|AC_EXP01|AC_EXP02|MC}` / `--eval-datasets {AC,AC_EXP01,AC_EXP02,MC,MB}` / `--variants` / `--epochs` 로 지정된 HF Hub merged repo (`SaFD-00/{short}-{slug}world-model-stage1-{MODE}-epoch{E}`) 를 pull 해 EVAL_DS 별 test JSONL 에 대해 `hungarian_metrics.json` 산출.
- EVAL_DS 별 분기:
...
- 흐름: `stage2_train.sh → stage2_merge.sh → stage2_eval.sh`. TRAIN_DATASET 은 `AC | AC_EXP01 | AC_EXP02` (MC 는 Stage 2 데이터 없음). AC_EXP01 는 `--exp01-ratio {r37|r55|r73}` 단일 ratio 로 학습 모델을 지정.
- EVAL_DS 별 분기:
...
- 산출 경로: `outputs/{OUT_DS}/eval/{MODEL}{SFX}/stage2_eval/{variant_path}[_from_{M1}-ep{E1}][/epoch-{E2}]/on-{EVAL_DS}/`. AC 는 OUT_DS=TRAIN_DS, SFX=`""`. AC_EXP01 는 OUT_DS=`AC_EXP01`, SFX=`_ratio{37,55,73}` (variant_path 는 CLI VARIANT 의 `world_model` → `world-model` 치환).
...
### shell 실행 공통 규약
- `AC` / `AC_EXP01` / `AC_EXP02` / `MC` / `MB` 매핑 (`DS_PREFIX` / `HF_SLUG` / `DS_DATADIR`), 모델 레지스트리 → [`scripts/_common.sh`](./scripts/_common.sh).
...
- 문서나 스크립트에서 `outputs/{DS}/{category}/...` 의 `{DS}` 는 `AC` / `AC_EXP01` / `AC_EXP02` / `MC`. AC_EXP01 의 ratio (`r37/r55/r73`) 는 디렉토리가 아니라 그 아래 모델 디렉토리의 suffix 로 운반되며 (Stage 1: `{MODEL}_ratio{37,55,73}_stage1_{MODE}_world-model/`, Stage 2: `{MODEL}_ratio{37,55,73}_stage2_{MODE2}_{base|world-model_from_{MODE1}-ep{E1}}/`), 모든 ratio 산출물이 `outputs/AC_EXP01/` 단일 부모 아래에 모인다. `{category}` 는 `adapters | eval | merged`. `adapters/` 는 flat 네이밍, `merged/` 는 `{MODEL}{SFX}_{detail}/epoch-{E}/` 로 epoch 별 서브디렉토리 분리. `eval/` 은 `{MODEL}{SFX}/stage{1,2}_eval/.../epoch-{E}/` 중첩 (AC 는 SFX=`""`, AC_EXP01 는 `_ratio{37,55,73}`). `BEST_CHECKPOINT` 파일은 더 이상 생성되지 않는다.
- `data/` 아래 실제 디렉토리명은 `AndroidControl`, `AndroidControl_EXP01`, `AndroidControl_EXP02`, `MonkeyCollection`, `MobiBench` (평가 전용). MobiBench 는 `implicit-world-modeling_stage{1,2}.jsonl` 두 단일 파일만.
...
- [`scripts/split_data.py`](./scripts/split_data.py) 는 Stage 1 + Stage 2 분할을 모두 담당. **AC**: Stage 1 / Stage 2 모두 `episodes_meta.jsonl.primary_app` 기반 app-level ID/OOD split, 단일 partition 공유. 산출 파일 `implicit-world-modeling_stage{1,2}_{train,test_id,test_ood}.jsonl`. **MC**: 메타 없음 → Stage 1 random split (`_train` / `_test`) 자동 fallback. MC 는 `_STAGE1_ONLY` 라 Stage 2 자동 skip. **MB**: split 미수행 (평가 전용). AC 메타는 [`scripts/extract_androidcontrol_metadata.py`](./scripts/extract_androidcontrol_metadata.py), 스크린샷은 [`scripts/extract_androidcontrol_images.py`](./scripts/extract_androidcontrol_images.py) 가 생성한다.
- bash 자동화는 bash 4+ 전제.
- shell script CLI 공통 플래그:
  - **학습/merge (`stage{1,2}_{train,merge}.sh`)**: `--model MODEL --dataset {AC|AC_EXP01|AC_EXP02|MC|all} --stage1-mode {full|lora} --exp01-ratios LIST`. `--dataset MB` 는 거절. `--dataset all` = AC + MC (AC_EXP01 / AC_EXP02 는 명시적). `stage2_*`: `--stage2-mode {full|lora}` (기본 lora), `--stage1-epoch N` (world-model variant 전용). AC_EXP01 sweep 부분 실행은 `--exp01-ratios r55,r73`.
  - **평가 (`stage{1,2}_eval.sh`)**: `--model MODEL --train-dataset {AC|AC_EXP01|AC_EXP02|MC} --eval-datasets LIST --stage1-mode ... --stage2-mode ... --stage1-epoch N --epochs LIST --variants LIST --exp01-ratio {r37|r55|r73}`. `--eval-datasets` 는 콤마 구분, 허용 `AC,AC_EXP01,AC_EXP02,MC,MB`, 기본값은 `--train-dataset` 단일값 (AC_EXP01 의 기본 eval 은 raw `AC_EXP01`). Stage 2 eval 은 `--train-dataset {AC|AC_EXP01|AC_EXP02}` 만 (MC 미지원). AC_EXP01 학습 모델 평가는 ratio 정확히 1 개 (`--exp01-ratio r55` 기본).
