...
- **YAML/실측**: EXP05 stage1 YAML 6 개 전부 `use_diff_token_weighted_loss: true` (노트북 Cell 5 가 SSoT). `token_weights` 는 train jsonl 인라인 필드라 `dataset_info.json` 컬럼 등록은 불필요 (`converter.py:226` 이 raw jsonl 에서 직접 읽음). train 실측 **44,670** 행 = state 31,221 + action 13,449, weight 값 분포는 state `{0.25, 1.0}` / action `{1.0}`.
