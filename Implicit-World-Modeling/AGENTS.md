...
5. **offload 분기를 `GPU_TYPE` 만 보고 짜지 마라 — `(GPU_TYPE, size_class)` 쌍이다.** **(A100|H100) × 3-4B** 만 offload 를 끄고 (half-batch 반감도 면제 → `pdbs=2`), **7-9B 와 RTX5090 은 여전히 offload** 다 (7B 는 offload 를 빼면 확정 OOM). 옛 `if GPU_TYPE == "RTX5090": …` 처럼 GPU 종류만 보는 분기는 7-9B 까지 no-offload 로 넘겨 조용히 OOM 내므로 **되살리지 마라.** `--allow-no-offload` 는 offload 를 켜는 조합에서 강제로 끄는 opt-out 이다. [§2 함정 7](./ARCHITECTURE.md#2-모델-설정)
