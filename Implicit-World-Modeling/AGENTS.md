...
- 모델 family 별 image budget 은 노트북 Cell 5 의 `MODEL_FAMILY_CONFIG` (factor / max_tokens / min_tokens) 와 `_DATASET_CONFIG[ds]["image_overrides"]` 의 token 단위 override 로 관리된다. token 예산은 학습 데이터셋으로 결정 — AC1·MC 학습은 family default `max_tokens=2048` (Qwen3-VL → 2,097,152), AC_2 학습은 override `max_tokens=5400` (5,529,600). 평가는 `TRAIN_DATASET` 으로 동일 budget 적용.
...
- **데이터셋 역할 분리**:
  - 학습 대상 DS: `AndroidControl` (AC), `AndroidControl_2` (AC_2), `AndroidControl_EXP01` (AC_EXP01), `MonkeyCollection` (MC).
...
### 하이퍼파라미터 수정
- AC 는 `_SIZE_CONFIG_AC[size].stage{1, 1_lora, 2}` 로 **크기 3 단 (2B / 3-4B / 7-9B)** 공유값 관리. `_MODEL_CONFIG[model].hparam_overrides` 는 모델별 delta 전용. lr / warmup / LoRA rank / dropout 은 `_MODEL_CONFIG` 에 직접 쓰지 말고 `_SIZE_CONFIG_AC` 에서 해당 tier 값을 바꾼다.
- AC_2 / MC 는 tier 미적용 — dataset baseline + per-model override 만 적용.
- MB 는 평가 전용이라 학습 하이퍼파라미터 해석에서 제외.
- merge 순서 (단일 진실원: 노트북 Cell 5 CONFIGS 빌더): `_DATASET_CONFIG` baseline → `_SIZE_CONFIG_AC[size]` (AC 일 때만) → `hparam_overrides`. 전체 표는 [`ARCHITECTURE.md`](./ARCHITECTURE.md) §2.
...
- AC_EXP01 분할: 선행으로 `python scripts/filter_long_samples.py --dataset AC_EXP01` 가 mm-expanded length > cutoff_len 샘플을 제거해 `implicit-world-modeling_stage1_{state,action}_pred_filtered.jsonl` + `implicit-world-modeling_stage2_filtered.jsonl` (3 파일) 을 만든다 (Qwen3-VL `get_rope_index` broadcast 회피). `--image-max-pixels` 기본값 2097152 는 Qwen3-VL family (factor 32) 기준 — Qwen2/2.5-VL 학습 시 1605632 등으로 override 필요. `--skip-existing` 으로 누락된 source 만 처리 가능 (이미 산출된 _filtered.jsonl 재처리 회피). 그 위에서 `split_data.py --dataset AC_EXP01 --exp01-ratios 7:3,3:7,5:5 --exp01-train-total 50000` (콜론 구분, default 동일) 가 `run_ac3_split` 으로 `implicit-world-modeling_stage1_train_{3_7,5_5,7_3}.jsonl` 3 개 + task × split 4 test (`implicit-world-modeling_stage1_test_{id,ood}_{state,action}_pred.jsonl`) 를 산출하고, 이어서 같은 (id_apps, ood_apps) partition 으로 `implicit-world-modeling_stage2_filtered.jsonl` 풀에서 Stage 2 split `implicit-world-modeling_stage2_{train,test_id,test_ood}.jsonl` (15K / 3K / 3K, action_type stratified) 까지 만든다 (Stage 2 _filtered 누락 시 hard-fail). `state_pred` 는 random, `action_pred` / Stage 2 는 action-type stratified 샘플링. Stage 2 마지막 메시지의 `<thought>…</thought>\n<action>{...}</action>` 래핑은 `_parse_action_payload` regex helper 가 분리해 `action_type` 을 추출한다.
...
- 흐름: **`train → merge → eval`** — `stage1_merge.sh` 가 모든 epoch 를 각각 local merge + HF push (`trainer_state.json.epoch` 파싱), `stage1_eval.sh` 가 `--train-dataset {AC|AC_2|MC}` / `--eval-datasets {AC,AC_2,MC,MB}` / `--variants` / `--epochs` 로 지정된 HF Hub merged repo (`SaFD-00/{short}-{slug}world-model-stage1-{MODE}-epoch{E}`) 를 pull 해 EVAL_DS 별 test JSONL 에 대해 `hungarian_metrics.json` 산출.
- EVAL_DS 별 분기:
  - **EVAL_DS=AC**: ID + OOD 두 test 파일 (`implicit-world-modeling_stage1_test_{id,ood}.jsonl`) 함께 추론 → 3 섹션.
  - **EVAL_DS=AC_2**: 단일 파일 `implicit-world-modeling_stage1_test.jsonl` (사전 분할) → single-pair overall.
  - **EVAL_DS=AC_EXP01**: state_pred + action_pred **두 task 를 각각 독립 채점**. 각 task 가 (id, ood) 2 파일을 가지므로 inference 4 회 → state 산출 `on-AC_EXP01-state/hungarian_metrics.json` (Stage1 채점, `_hungarian_eval.py`), action 산출 `on-AC_EXP01-action/action_metrics.json` (Stage2 채점, `_action_eval.py`). without_open_app sibling 은 state branch 만 (action 채점기 미지원).
...
- 흐름: `stage2_train.sh → stage2_merge.sh → stage2_eval.sh`. TRAIN_DATASET 은 `AC | AC_2 | AC_EXP01` (MC 는 Stage 2 데이터 없음). AC_EXP01 는 `--exp01-ratio {r37|r55|r73}` 단일 ratio 로 학습 모델을 지정.
- EVAL_DS 별 분기:
  - **EVAL_DS=AC / AC_EXP01**: ID + OOD 두 test 파일 (`implicit-world-modeling_stage2_test_{id,ood}.jsonl`) 함께 추론 → `action_metrics.json` 에 `overall` / `in_domain` / `out_of_domain` 3 섹션.
  - **EVAL_DS=AC_2**: 단일 파일 `implicit-world-modeling_stage2_test.jsonl` 1 회 추론 → single-pair `overall` 1 섹션.
...
- 산출 경로: `outputs/{OUT_DS}/eval/{MODEL}{SFX}/stage2_eval/{variant_path}[_from_{M1}-ep{E1}][/epoch-{E2}]/on-{EVAL_DS}/`. AC/AC_2 는 OUT_DS=TRAIN_DS, SFX=`""`. AC_EXP01 는 OUT_DS=`AC_EXP01`, SFX=`_ratio{37,55,73}` (variant_path 는 CLI VARIANT 의 `world_model` → `world-model` 치환).
...
- 문서나 스크립트에서 `outputs/{DS}/{category}/...` 의 `{DS}` 는 `AC` / `AC_2` / `AC_EXP01` / `MC`. AC_EXP01 의 ratio (`r37/r55/r73`) 는 디렉토리가 아니라 그 아래 모델 디렉토리의 suffix 로 운반되며 (Stage 1: `{MODEL}_ratio{37,55,73}_stage1_{MODE}_world-model/`, Stage 2: `{MODEL}_ratio{37,55,73}_stage2_{MODE2}_{base|world-model_from_{MODE1}-ep{E1}}/`), 모든 ratio 산출물이 `outputs/AC_EXP01/` 단일 부모 아래에 모인다. `{category}` 는 `adapters | eval | merged`. `adapters/` 는 flat 네이밍, `merged/` 는 `{MODEL}{SFX}_{detail}/epoch-{E}/` 로 epoch 별 서브디렉토리 분리. `eval/` 은 `{MODEL}{SFX}/stage{1,2}_eval/.../epoch-{E}/` 중첩 (AC/AC_2 는 SFX=`""`, AC_EXP01 는 `_ratio{37,55,73}`). `BEST_CHECKPOINT` 파일은 더 이상 생성되지 않는다.
- `data/` 아래 실제 디렉토리명은 `AndroidControl`, `AndroidControl_2`, `MonkeyCollection`, `MobiBench` (평가 전용). MobiBench 는 `implicit-world-modeling_stage{1,2}.jsonl` 두 단일 파일만. AndroidControl_2 는 `images/` 디렉토리 없이 AC 의 `images/` 를 JSONL `images` 필드로 참조 (canonical prefix `AndroidControl/images/...`).
...
- Stage 2 train/merge (world-model variant) 는 `--stage1-epoch N` 으로 지정된 로컬 `outputs/{OUT_DS}/merged/{MODEL}{SFX}_stage1_${STAGE1_MODE}_world-model/epoch-${N}/` 가 선행돼야 한다 (AC_EXP01 ratio variant 는 SFX=`_ratio{37,55,73}`). Stage 2 train 은 YAML 의 `model_name_or_path` 를 런타임에 sed 치환하므로 노트북 YAML 생성 시 placeholder 값 (HF id) 은 무시된다. Stage 2 eval 은 HF Hub merged repo 만 pull 하고 `--stage1-epoch` 값을 HF 레포명 계보 번호로 주입.
...
- [`scripts/split_data.py`](./scripts/split_data.py) 는 Stage 1 + Stage 2 분할을 모두 담당. **AC**: Stage 1 / Stage 2 모두 `episodes_meta.jsonl.primary_app` 기반 app-level ID/OOD split, 단일 partition 공유. 산출 파일 `implicit-world-modeling_stage{1,2}_{train,test_id,test_ood}.jsonl`. **MC**: 메타 없음 → Stage 1 random split (`_train` / `_test`) 자동 fallback. MC 는 `_STAGE1_ONLY` 라 Stage 2 자동 skip. **AC_2 / MB**: split 미수행. AC 메타는 [`scripts/extract_androidcontrol_metadata.py`](./scripts/extract_androidcontrol_metadata.py), 스크린샷은 [`scripts/extract_androidcontrol_images.py`](./scripts/extract_androidcontrol_images.py) 가 생성한다.
- bash 자동화는 bash 4+ 전제.
- shell script CLI 공통 플래그:
  - **학습/merge (`stage{1,2}_{train,merge}.sh`)**: `--model MODEL --dataset {AC|AC_2|AC_EXP01|MC|all} --stage1-mode {full|lora} --exp01-ratios LIST`. `--dataset MB` 는 거절. `--dataset all` = AC + AC_2 + MC (AC_EXP01 는 명시적). `stage2_*`: `--stage2-mode {full|lora}` (기본 lora), `--stage1-epoch N` (world-model variant 전용). AC_EXP01 sweep 부분 실행은 `--exp01-ratios r55,r73`.
  - **평가 (`stage{1,2}_eval.sh`)**: `--model MODEL --train-dataset {AC|AC_2|AC_EXP01|MC} --eval-datasets LIST --stage1-mode ... --stage2-mode ... --stage1-epoch N --epochs LIST --variants LIST --exp01-ratio {r37|r55|r73}`. `--eval-datasets` 는 콤마 구분, 허용 `AC,AC_2,AC_EXP01,MC,MB`, 기본값은 `--train-dataset` 단일값 (AC_EXP01 의 기본 eval 은 raw `AC_EXP01`). Stage 2 eval 은 `--train-dataset {AC|AC_2|AC_EXP01}` 만 (MC 미지원). AC_EXP01 학습 모델 평가는 ratio 정확히 1 개 (`--exp01-ratio r55` 기본).
...
- `bash scripts/stage1_train.sh --dataset AC_2 --model qwen3-vl-8b --help 2>&1` — AC_2 인식 확인 (exit 0)
- HF naming 단위 검증:
  ```bash
  source scripts/_common.sh && parse_args
  hf_repo_id_stage1 qwen3-vl-8b AC full 3
  # → SaFD-00/qwen3-vl-8b-ac-world-model-stage1-full-epoch3
  hf_repo_id_stage1 qwen3-vl-8b AC_2 full 3
  # → SaFD-00/qwen3-vl-8b-ac-2-world-model-stage1-full-epoch3
  hf_repo_id_stage2_world_model qwen3-vl-8b AC full 3 lora 1
  # → SaFD-00/qwen3-vl-8b-ac-world-model-stage1-full-epoch3-stage2-lora-epoch1
  ```
...
- 새 dataset 의 image budget 이 family default 와 달라야 하면 `_DATASET_CONFIG[ds]["image_overrides"]` 에 token 단위로 둔다 (`{"max_tokens": N}`) — 빌더가 family `factor²` 로 환산. 평가측은 `scripts/_common.sh::build_infer_cmd` 의 `TRAIN_DATASET` 분기를 갱신해 학습 DS 와 동일 budget 사용 (예: AC2 학습 → `max_tokens=5400`, family 별 4,233,600 / 5,529,600).
