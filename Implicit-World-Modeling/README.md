...
| 5 | Stage 1 평가 (local merged 우선 + HF Hub fallback sweep, EVAL_DS = AC_EXP01, AC_EXP02, MC, MB) |
...
```
Implicit-World-Modeling/outputs/{AndroidControl_EXP01|AndroidControl_EXP02|MC}/      # AC_EXP01 의 ratio (ratio37/ratio55/ratio73) 는 별도 디렉토리가 아니라 아래 {model}{SFX} 의 SFX 로 운반된다 (AC_EXP02/MC: SFX="", AC_EXP01: SFX=_ratio37/_ratio55/_ratio73).
├── adapters/
...
`{slug}` 는 `ac-exp01-ratio{37,55,73}-` (AC_EXP01 ratio 별) / `ac-exp02-` (AC_EXP02) / `mc-` (MC). `{E}` 는 `trainer_state.json.epoch` 의 `int(round(...))`. HF repo id 조립은 `scripts/_common.sh::hf_repo_id_stage1` / `hf_repo_id_stage2_base` / `hf_repo_id_stage2_world_model` 헬퍼에 단일화되어 있고, eval 시 사용할 model path 는 `resolve_eval_model_path {stage1|stage2_base|stage2_world}` 가 **local merged dir (`outputs/.../merged/.../epoch-{E}/`) 우선 + 위 HF repo id fallback** 으로 해석한다.
