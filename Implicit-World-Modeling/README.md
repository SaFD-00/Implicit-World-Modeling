...
├── data/                         # AndroidControl (원본 source) / AC_EXP01 / AC_EXP02 / MC / MB
├── LlamaFactory/                 # 학습/추론 엔진 (clone)
│   ├── examples/custom/IWM-{AC_EXP01,AC_EXP02,MC}/  # 노트북이 생성한 학습 YAML
...
학습 대상 DS 는 **AC_EXP01**, **AC_EXP02**, **MC**. **MB** 는 평가 전용이므로 split 하지 않는다. **AndroidControl (AC) 자체는 학습/평가 대상이 아니라 AC_EXP01 / AC_EXP02 의 원본 source 자산** (이미지 + 원본 jsonl + `episodes_meta.jsonl`) 으로만 보존된다.

```
data/
├── AndroidControl/                 # 원본 source (이미지 + jsonl + episodes_meta) — EXP01/EXP02 가 read
├── AndroidControl_EXP01/           # state+action ratio-mix (Stage 1 + 2, ID/OOD) — split 산출물
├── AndroidControl_EXP02/           # AC_EXP01 r73 + diff loss 실험군 (Stage 1 + 2) — 산출물
...
`AndroidControl` 폴더는 학습/평가 entry 가 아니다 — AC_EXP01 / AC_EXP02 가 source 로 read 한다.
...
| 1-2 | `dataset_info.json` 등록 (AC_EXP01 / AC_EXP02 / MC / MB) |
| 3 | Stage 1 SFT 학습 (`qwen3-vl-8b` + Full FT, AC_EXP01 · AC_EXP02) |
| 4 | Stage 1 merge (`qwen3-vl-8b` + Full FT, 모든 epoch local merge + 선택적 HF Hub push; `--no-hf-upload` 지원) |
| 5 | Stage 1 평가 (HF Hub merged sweep, EVAL_DS = AC_EXP01, AC_EXP02, MC, MB) |
| 6 | Stage 2 SFT 학습 (`qwen3-vl-8b` + LoRA, AC_EXP01 ratio sweep · AC_EXP02) |
...
|---|---|---|---|
| `stage1_eval.sh` | `AC_EXP01` / `AC_EXP02` / `MC` | `AC_EXP01` / `AC_EXP02` / `MC` / `MB` | `base, full_world_model, lora_world_model` |
| `stage2_eval.sh` | `AC_EXP01` / `AC_EXP02` (MC 거절) | `AC_EXP01` / `AC_EXP02` / `MB` | `base, full_base, lora_base, full_world_model, lora_world_model` |
...
```
Implicit-World-Modeling/outputs/{AndroidControl_EXP01|AndroidControl_EXP02|MC}/      # AC_EXP01 의 ratio (r37/r55/r73) 는 별도 디렉토리가 아니라 아래 {model}{SFX} 의 SFX 로 운반된다 (AC_EXP02/MC: SFX="", AC_EXP01: SFX=_ratio37/_ratio55/_ratio73).
├── adapters/
...
`{slug}` 는 `ac-exp01-r{37,55,73}-` (AC_EXP01 ratio 별) / `ac-exp02-` (AC_EXP02) / `mc-` (MC). `{E}` 는 `trainer_state.json.epoch` 의 `int(round(...))`. 조립은 `scripts/_common.sh::hf_repo_id_stage1` / `hf_repo_id_stage2_base` / `hf_repo_id_stage2_world_model` 헬퍼에 단일화.
