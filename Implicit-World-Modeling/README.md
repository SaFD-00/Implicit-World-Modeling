...
모바일 GUI World Modeling 이 Action Prediction 성능에 미치는 영향을 검증하는 **2-stage fine-tuning 파이프라인**. 8 개 Qwen 계열 Vision-Language 모델 × 3 개 학습 데이터셋 매트릭스를 단일 conda env (`implicit-world-modeling`) + 저장소 내부 [`LlamaFactory/`](./LlamaFactory) 백엔드로 운영한다. 노트북 [`implicit-world-modeling.ipynb`](./implicit-world-modeling.ipynb) 가 기준 실행 경로이고 [`scripts/`](./scripts) 가 반복 실행용 자동화 레이어다.
...
|---|---|---|
| AC, MC | 2,048 (family default) | 2,097,152 (= 2048 × 32²) |
| AC_2 | **5,400 (dataset override)** | 5,529,600 (= 5400 × 32²) |
...
│   ├── examples/custom/IWM-{AC,AC_2,MC}/  # 노트북이 생성한 학습 YAML
...
#   → implicit-world-modeling_stage{1,2}_{train,test_id,test_ood}.jsonl
```

### 2) AndroidControl_2 준비

사전 분할 데이터로 제공되므로 `split_data.py` 를 다시 돌리지 않는다. `data/AndroidControl_2/implicit-world-modeling_stage{1,2}_{train,test}.jsonl` 이 이미 있어야 한다. JSONL `images` 가 `AndroidControl/images/...` 를 참조하므로 **AC images 가 살아 있어야 한다.**

### 2.5) AndroidControl_EXP01 준비 (Stage 1 state+action ratio mix + Stage 2 ID/OOD split)
...
# Stage 1 → implicit-world-modeling_stage1_train_{3_7,5_5,7_3}.jsonl
#         + implicit-world-modeling_stage1_test_{id,ood}_{state,action}_pred.jsonl
# Stage 2 → implicit-world-modeling_stage2_{train,test_id,test_ood}.jsonl   (15K / 3K / 3K, _filtered 풀에서 산출, Stage 1 action_pred app partition 공유)
```

ratio 별로 **별개의 학습 가중치** 가 산출되므로 train/merge/eval 모두 `--dataset AC_EXP01` 한 번에 ratio 3 종을 sweep 한다 (`--exp01-ratios r55,r73` 로 부분 실행 가능). 평가는 ratio 단일 (`--exp01-ratio r55` 기본) 로 고정. **Stage 1** 은 state/action **두 task 를 각각 채점** (`_hungarian_eval.py` / `_action_eval.py`). **Stage 2** 도 같은 ratio sweep 으로 활성 — Stage 1 ratio merged 를 base 로 같은 `implicit-world-modeling_stage2_{train,test_id,test_ood}.jsonl` 을 학습. stage2 데이터 자체는 ratio 와 무관 (3 ratio 공유) 이며 ratio 차원은 stage1 → stage2 base 계보로만 흐른다 (산출 디렉토리/HF slug 는 ratio 별 분리).

### 2.6) AndroidControl_EXP02 준비 (AC_EXP01 r73 + Stage 1 diff loss 실험군)

`AndroidControl_EXP02` 는 **AC_EXP01 r73 과 동일한 학습 데이터·하이퍼파라미터**로 학습하되, Stage 1 state prediction 에 **diff loss (token-weighted SFT)** 를 적용하는 실험군이다. AC_EXP01 r73 (기존 균일 cross-entropy) 대조군과 직접 비교해 diff loss 의 순효과를 측정한다.
...
- **데이터**: train 만 별도 — `scripts/diff_loss/preprocess_dataset.py` 가 AC_EXP01 r73 train 에 `token_weights` 필드를 추가. test / Stage 2 데이터는 AC_EXP01 에서 복사 (동일 평가셋 — 공정 비교).
...
- **AC_EXP02 (diff loss 실험군)**: split 불필요 — AC_EXP01 r73 train 에 `token_weights` 부여 (`scripts/diff_loss/preprocess_dataset.py`) + test/Stage 2 는 AC_EXP01 복사. 노트북 Section 0 환경 세팅 셀이 일괄 수행.
- **AC_2 / MB**: split 불필요.
...
| 3 | Stage 1 SFT 학습 (`qwen3-vl-8b` + Full FT, AC · AC_2 · AC_EXP01) |
| 4 | Stage 1 merge (`qwen3-vl-8b` + Full FT, 모든 epoch local merge + 선택적 HF Hub push; `--no-hf-upload` 지원) |
| 5 | Stage 1 평가 (HF Hub merged sweep, EVAL_DS = AC, AC_2, AC_EXP01, MC, MB) |
| 6 | Stage 2 SFT 학습 (`qwen3-vl-8b` + LoRA, AC · AC_2 · AC_EXP01 ratio sweep) |
...
bash scripts/stage1_merge.sh --model qwen3-vl-8b --dataset AC_EXP01 --stage1-mode lora \
     --exp01-ratios r37,r73 --no-hf-upload
...
bash scripts/stage1_train.sh --model qwen3-vl-8b --dataset AC_EXP01 --exp01-ratios r55,r73 --stage1-mode lora
bash scripts/stage1_eval.sh  --model qwen3-vl-8b --train-dataset AC_EXP01 --exp01-ratio r73 \
     --eval-datasets AC_EXP01,AC,MB

# 전체 모델 × 전체 DS 일괄 sweep (--model / --dataset 생략)
bash scripts/stage1_train.sh --stage1-mode full   # 8 모델 × {AC, AC_2, MC} (AC_EXP01 는 명시적)
bash scripts/stage1_merge.sh --stage1-mode full   # 학습 안 된 슬롯은 [WARN] SKIP

# Stage 2 — AC, AC_2, AC_EXP01 지원 (MC 는 Stage 2 데이터/YAML 없음 → 거절)
...
|---|---|---|---|
| `stage1_eval.sh` | `AC` / `AC_2` / `AC_EXP01` / `MC` | `AC` / `AC_2` / `AC_EXP01` / `MC` / `MB` | `base, full_world_model, lora_world_model` |
| `stage2_eval.sh` | `AC` / `AC_2` / `AC_EXP01` (MC 거절) | `AC` / `AC_2` / `AC_EXP01` / `MB` | `base, full_base, lora_base, full_world_model, lora_world_model` |
...
```
Implicit-World-Modeling/outputs/{AC|AC_2|AC_EXP01|MC}/      # AC_EXP01 의 ratio (r37/r55/r73) 는 별도 디렉토리가 아니라 아래 {model}{SFX} 의 SFX 로 운반된다 (AC/AC_2/MC: SFX="", AC_EXP01: SFX=_ratio37/_ratio55/_ratio73).
├── adapters/
...
`{slug}` 는 `ac-` (AC) / `ac-2-` (AC_2) / `ac-exp01-r{37,55,73}-` (AC_EXP01 ratio 별) / `mc-` (MC). `{E}` 는 `trainer_state.json.epoch` 의 `int(round(...))`. 조립은 `scripts/_common.sh::hf_repo_id_stage1` / `hf_repo_id_stage2_base` / `hf_repo_id_stage2_world_model` 헬퍼에 단일화.
