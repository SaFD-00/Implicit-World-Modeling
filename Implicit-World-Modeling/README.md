...
|---|---|---|
| AC, AC_EXP01, AC_EXP02, MC | 2,048 (family default) | 2,097,152 (= 2048 × 32²) |
...
│   ├── split_data.py             # AC/MC split (AC_EXP01 는 ratio-mix split)
...
├── data/                         # AC / AC_EXP01 / AC_EXP02 / MC / MB
├── LlamaFactory/                 # 학습/추론 엔진 (clone)
│   ├── examples/custom/IWM-{AC,AC_EXP01,AC_EXP02,MC}/  # 노트북이 생성한 학습 YAML
...
학습 대상 DS 는 **AC**, **AC_EXP01**, **AC_EXP02**, **MC**. **MB** 는 평가 전용이므로 split 하지 않는다.

```
data/
├── AndroidControl/                 # 학습 + 평가 (Stage 1 + 2, ID/OOD)
├── AndroidControl_EXP01/           # state+action ratio-mix (Stage 1 + 2, ID/OOD)
├── AndroidControl_EXP02/           # AC_EXP01 r73 + diff loss 실험군 (Stage 1 + 2)
...
### 3) AndroidControl_EXP02 준비 (AC_EXP01 r73 + Stage 1 diff loss 실험군)
...
| 1-2 | `dataset_info.json` 등록 (AC / AC_EXP01 / AC_EXP02 / MC / MB) |
| 3 | Stage 1 SFT 학습 (`qwen3-vl-8b` + Full FT, AC · AC_EXP01 · AC_EXP02) |
| 4 | Stage 1 merge (`qwen3-vl-8b` + Full FT, 모든 epoch local merge + 선택적 HF Hub push; `--no-hf-upload` 지원) |
| 5 | Stage 1 평가 (HF Hub merged sweep, EVAL_DS = AC, AC_EXP01, AC_EXP02, MC, MB) |
| 6 | Stage 2 SFT 학습 (`qwen3-vl-8b` + LoRA, AC · AC_EXP01 ratio sweep · AC_EXP02) |
...
```bash
# Stage 1 Full FT — train → merge → eval (AC 학습, 교차 평가)
...
bash scripts/stage1_train.sh --stage1-mode full   # 모델 × {AC, MC} (AC_EXP01/AC_EXP02 는 명시적)
bash scripts/stage1_merge.sh --stage1-mode full   # 학습 안 된 슬롯은 [WARN] SKIP

# Stage 2 — AC, AC_EXP01, AC_EXP02 지원 (MC 는 Stage 2 데이터/YAML 없음 → 거절)
...
|---|---|---|---|
| `stage1_eval.sh` | `AC` / `AC_EXP01` / `AC_EXP02` / `MC` | `AC` / `AC_EXP01` / `AC_EXP02` / `MC` / `MB` | `base, full_world_model, lora_world_model` |
| `stage2_eval.sh` | `AC` / `AC_EXP01` / `AC_EXP02` (MC 거절) | `AC` / `AC_EXP01` / `AC_EXP02` / `MB` | `base, full_base, lora_base, full_world_model, lora_world_model` |
...
`--data-dir AC` 로 데이터/산출물 루트를 선택 (현재 기본이자 유일 값). 산출 위치: `outputs/AC/eval/{MODEL}/stage{1,2}_eval/{pairs_on-AC.html, pairs_on-MB.html, pairs_summary.md}`.

## 산출물

```
Implicit-World-Modeling/outputs/{AC|AC_EXP01|AC_EXP02|MC}/      # AC_EXP01 의 ratio (r37/r55/r73) 는 별도 디렉토리가 아니라 아래 {model}{SFX} 의 SFX 로 운반된다 (AC/AC_EXP02/MC: SFX="", AC_EXP01: SFX=_ratio37/_ratio55/_ratio73).
├── adapters/
...
`{slug}` 는 `ac-` (AC) / `ac-exp01-r{37,55,73}-` (AC_EXP01 ratio 별) / `ac-exp02-` (AC_EXP02) / `mc-` (MC). `{E}` 는 `trainer_state.json.epoch` 의 `int(round(...))`. 조립은 `scripts/_common.sh::hf_repo_id_stage1` / `hf_repo_id_stage2_base` / `hf_repo_id_stage2_world_model` 헬퍼에 단일화.
