...
> **EXP05 실험군은 Qwen2.5-VL 전용** (`qwen2.5-vl-3b`/`qwen2.5-vl-7b`): 절대 픽셀 좌표(840×1876) 데이터라 Qwen2.5-VL native 와 일치하고, EXP03/EXP04 (0–1000 정규화) 는 반대로 `qwen3-vl-8b` 전용이다. 사용자 요청의 "3B/8B 모두" 에서 **8B = `qwen2.5-vl-7b`** (Qwen2.5-VL 에 8B 부재; `qwen3-vl-8b` 는 factor 32·정규화 native 로 EXP05 부적합).
...
|---|---|---|---|
| AC_EXP01, AC_EXP02, AC_EXP03, AC_EXP04, MC | 2,048 (family default) | 2,097,152 (= 2048 × 32²) | 1,605,632 (= 2048 × 28²) |
| AC_EXP05 (Qwen2.5-VL 전용) | 2,048 (family default) | — (미사용) | **1,605,632** (데이터 생성 budget 과 일치 → grounding 정합) |
...
│   ├── mirror_exp04.py           # AC_EXP04 = AC_EXP03 멤버십·좌표 + Stage 1 프롬프트 업그레이드 미러
│   ├── mirror_exp05.py           # AC_EXP05 = AC_EXP01 ratio73 멤버십 + 절대 픽셀 좌표(840×1876) 미러 (Qwen2.5-VL 전용, stage1-only)
...
│   ├── examples/custom/IWM-{AC_EXP01,AC_EXP02,AC_EXP03,AC_EXP04,MC}/  # 노트북이 생성한 학습 YAML (AC_EXP04 는 EXP03 stage1 YAML 복사본)
...
> **AC_EXP03 / AC_EXP04 / AC_EXP05 예외**: 좌표(point) 표현으로 시퀀스가 ~2.5x (`cutoff_len 24576`) 길어 메모리가 커지므로, 좌표 표현 실험군 (EXP03 · EXP04 · EXP05) 만 `per_device_train_batch_size` 를 절반 (H100/A100 2→1, RTX5090 1 유지) 으로 낮추고 `gradient_accumulation_steps` 로 보정해 `GLOBAL_BATCH_SIZE=64` 를 유지한다 (EXP01 과 global batch 동일 → 표현 차이만 공정 비교). EXP04 는 EXP03 의 stage1 YAML·batch 설정을 그대로 복사해 쓰며, GPU 0,1 (NPROC=2) 2-GPU 스케줄에서 LoRA 는 `gradient_accumulation_steps 32` (global batch 64), Full 은 `16` (global batch 32) 으로 둔다.
...
├── AndroidControl_EXP05/           # AC_EXP01 ratio73 멤버십 + 절대 픽셀 좌표(840×1876) 미러 (Stage 1 전용, Qwen2.5-VL 전용) — 산출물, 총 64,787 행
...
- **미러**: `scripts/mirror_exp04.py` 가 EXP01 ratio73 (= EXP03 와 동일) 멤버십을 따라 `data/AndroidControl_EXP04/` 에 stage1 train (49,276 줄) + dual-task test 6 종을 생성한다.
...
```bash
python scripts/mirror_exp04.py
...
- **cutoff_len 24576 공유 · half-batch**: 좌표(point) 표현이라 EXP03/EXP04 와 동일. image budget 은 Qwen2.5-VL family 기본값(1,605,632/3,136)과 같아 `image_overrides` 불필요 — 데이터 생성 budget 과 학습 프로세서 `image_max_pixels` 가 자동 일치.
...
```bash
# 1) Drive '0710_버젼' 에서 소스 2 파일 다운로드 후 canonical 이름으로 배치 (예: gdown --folder <folder-url>)
#    → data/AndroidControl/implicit-world-modeling_stage1_{action,state}_xy_pixel-aligned.jsonl
python scripts/mirror_exp05.py
# → data/AndroidControl_EXP05/implicit-world-modeling_stage1_train.jsonl  (47,556 줄)
#   + stage1_test_{id,ood}_{state,action}(+_without_open_app)   (총 7 파일, 64,787 행)
...
- **AC_EXP04 (프롬프트 업그레이드 실험군, Stage 1 전용)**: split 불필요 — `scripts/mirror_exp04.py` 가 AC_EXP03 (= AC_EXP01 ratio73) 의 stage1 train + dual-task test 멤버십을 좌표 표현 그대로 미러하되 프롬프트만 업그레이드 (`swipe` / html-style-XML / `[SWIPE]`). EXP04 pool ⊆ EXP03 pool → train 320 / 전체 stage1 450 (0.67%) drop. 프롬프트 품질 효과만 EXP03 과 비교 (Stage 2 보류).
- **AC_EXP05 (절대 픽셀 좌표 실험군, AndroidWorld 해상도 정렬, Stage 1 전용)**: split 불필요 — `scripts/mirror_exp05.py` 가 AC_EXP01 ratio73 stage1 멤버십을 **절대 픽셀 좌표(840×1876, budget 1,605,632, factor 28)** 로 미러 (없는 키 제외, 입력 50,000 → train 47,556 / 총 64,787). **Qwen2.5-VL 전용** (`qwen2.5-vl-3b`/`qwen2.5-vl-7b`), Stage 2 보류. 해상도/좌표계 정렬 효과를 정규화 실험군과 대비.
- **MB**: split 불필요.
...
3. 노트북 Section 0 의 "Stage {1,2} YAML 일괄 생성" 셀 (Cell 8 / 10) 재실행 → YAML 자동 생성. MC 와 AC_EXP04 는 Stage 1 전용이라 Stage 2 YAML 이 생성되지 않는다 (`_STAGE1_ONLY = {"MonkeyCollection", "AndroidControl_EXP04"}` guard).
...
- [`scripts/mirror_exp04.py`](./scripts/mirror_exp04.py) — AC_EXP04 프롬프트 업그레이드 미러 (AC_EXP03 멤버십·좌표)
- [`scripts/mirror_exp05.py`](./scripts/mirror_exp05.py) — AC_EXP05 절대 픽셀 좌표(840×1876) 미러 (AC_EXP01 ratio73 멤버십, Qwen2.5-VL 전용, AndroidWorld 해상도 정렬)
