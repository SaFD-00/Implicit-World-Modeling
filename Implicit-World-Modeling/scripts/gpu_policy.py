...
★ offload 정책 — 80GB GPU × 3-4B 만 끈다
--------------------------------------------------------------------------
``resolve_gpu_policy`` 는 **(A100 | H100) × 3-4B** 조합에서만 offload 를 끄고
(``ds_z3_config.json``), 그 밖의 모든 조합에서는 ``ds_z3_offload_config.json``
(CPU offload 활성 DeepSpeed ZeRO-3 config) 를 반환한다.
...
(a) offload 켠 상태의 step 시간이 **165 s/step** 으로, 2094 step 이면 약 4 일이
    ...
    기다리는 시간이 step 을 지배한다는 신호다.
(b) 3-4B 모델은 optimizer state 를 GPU 에 올려도 80GB 안에 들어간다. 파라미터
    3.09 B 기준 fp32 master + Adam m/v 를 2 GPU 로 샤딩하면 GPU 당 ~25 GB 이고,
    여기에 (a) 에서 실측한 activation/logits 23~26 GB 를 더해도 여유가 있다.

offload 를 **유지해야 하는** 조합과 그 근거:

(c) **7-9B**: A100/H100 에서 offload 를 빼면 EXP05 7B full FT 는 **확정 OOM**
    이다 (실측: 2GPU 기준 모델 상태만 GPU 당 ~77 GiB). OOM peak 을 지배하는
    항은 lm_head logits (시퀀스 길이 × vocab 크기) 이며, 이는 파라미터 샤딩이나
    GPU 대수 증설로 줄지 않는다 — "GPU 가 많으니 괜찮을 것" 은 성립하지 않는다.
(d) **RTX5090 (32GB)**: 크기와 무관하게 offload 없이는 들어가지 않는다.
...
조용히 divergence 했다. 지금의 분기는 **(gpu_type, size_class) 쌍**을 함께 보고,
근거는 (a)~(d) 로 각 조합마다 실측에 묶여 있다.

``allow_no_offload=True`` opt-out 은 그대로 남는다 — 위 정책이 offload 를 켜는
조합(7-9B, RTX5090)에서 강제로 끄는 용도이며 미실측 경고를 동반한다.
...
# 80GB GPU × 3-4B — offload 를 끄고 half-batch 예외도 면제하는 유일한 조합.
# 근거는 모듈 docstring (a)/(b). 이 두 결정은 같은 실측에서 나오므로 함께 판정한다.
...
def _is_large_mem_small_model(gpu_type: str, size_class: str) -> bool:
    """80GB GPU (A100/H100) 에서 3-4B 모델을 돌리는 조합인가.

    이 조합만 (1) offload 를 끄고 (2) 긴 시퀀스 실험군의 half-batch 예외를
    면제한다 — 둘 다 모듈 docstring (a)/(b) 의 같은 실측에 근거한다.
    """
    return gpu_type in _LARGE_MEM_GPU_TYPES and size_class == _NO_OFFLOAD_SIZE_CLASS
...
def resolve_gpu_policy(
    ...
    size_class : {"7-9B", "3-4B"} — ``3-4B`` × (A100|H100) 은 offload 를 끄고
        half-batch 예외도 면제한다 (모듈 docstring (a)/(b)).
    ds_name : 데이터셋 키. ``AndroidControl_EXP03/04/05`` 는 긴 시퀀스 예외로
        ``per_device_train_batch_size`` 를 절반으로 낮춘다 — 단 80GB × 3-4B 는
        면제 (그 외 값은 자유 문자열).
    mode : {"full", "lora"}
    allow_no_offload : 정책이 offload 를 켜는 조합(7-9B / RTX5090)에서 강제로
        끈다 (opt-out — 미실측 경고 동반). 80GB × 3-4B 는 이 플래그와 무관하게
        이미 offload 가 꺼져 있다.
    ...
    large_mem_small_model = _is_large_mem_small_model(gpu_type, size_class)

    per_device = _BASE_PER_DEVICE_BS[gpu_type]
    # 80GB × 3-4B 는 offload 를 끄고도 여유가 있어 half-batch 예외를 면제한다.
    if ds_name in _HALF_BATCH_DATASETS and not large_mem_small_model:
    ...
    warnings: list[str] = []
    if large_mem_small_model:
        # 정책 경로 — 실측 근거가 있으므로 경고를 붙이지 않는다.
        deepspeed_path = DEEPSPEED_NO_OFFLOAD
        offload = False
    elif allow_no_offload:
        # opt-out — 정책이 offload 를 켜는 조합(7-9B / RTX5090)에서 강제로 끈다.
        deepspeed_path = DEEPSPEED_NO_OFFLOAD
        offload = False
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    # opt-out — 정책이 offload 를 켜는 조합(7-9B / RTX5090)에서 강제로 끈다.
    # 80GB × 3-4B 는 이 플래그 없이도 정책상 이미 offload 가 꺼진다.
    parser.add_argument(
        "--allow-no-offload",
        action="store_true",
        help=(
            "7-9B / RTX5090 조합에서 DeepSpeed offload 를 강제로 끈다 "
            "(opt-out, 미실측). 80GB × 3-4B 는 기본이 이미 no-offload."
        ),
    )
