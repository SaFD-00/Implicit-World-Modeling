...
def _warn_coord_space(points):
    ...
    n_oor = sum(1 for x, y in points
                if not (0 <= x <= XY_SCREEN_W and 0 <= y <= XY_SCREEN_H))
...
def evaluate_single_xy(gt_action, pred_action, ui_xml):
    ...
    if gt_type in ('click', 'long_press'):
        ...
        x1, y1, x2, y2 = bbox
        result['step_correct'] = (
            x1 <= pred_pt[0] <= x2 and y1 <= pred_pt[1] <= y2
        )
        return result
...
def _evaluate_pairs_xy(gt_entries, pred_entries):
    ...
    for gt_entry, pred_entry in zip(gt_entries, pred_entries):
        ...
        for flag, key in (('has_bbox_check', 'bbox'), ('has_dir_check', 'dir'),
                          ('has_app_check', 'app'), ('has_text_check', 'text')):
            if r[flag]:
                cond[key]['n'] += 1
...
def main():
    ...
    p_s.add_argument(
        "--coord-mode", default="index", choices=["index", "xy"], dest="coord_mode",
