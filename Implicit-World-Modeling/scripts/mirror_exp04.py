#!/usr/bin/env python3
"""EXP01 ratio73 멤버십을 EXP04 (좌표/point 표현 + 프롬프트 업그레이드) 으로 미러링한다.

EXP04 는 EXP03 와 **동일한 (episode, step) 멤버십·좌표(point) 표현** 을 유지하되,
stage1 프롬프트만 업그레이드한 실험군이다:
  - action space: ``scroll(direction, point)`` → ``swipe(start, end)``
  - role 문구: "represented as html-style XML" 명시
  - ``[SWIPE]`` 규칙 추가

EXP04 원천 2종 (data/AndroidControl/) 은 EXP03 좌표 pool 의 프롬프트 업그레이드본:
  - implicit-world-modeling_stage1_action_xy_prompt-enhanced.jsonl   (action_pred)
  - implicit-world-modeling_stage1_state_xy_prompt-enhanced.jsonl    (state_pred)
...
대응하는 EXP04 좌표 레코드를 동일 순서로 골라 출력한다 — EXP01/EXP02/EXP03 과 **행 단위
1:1 대응** (표현만 좌표 + 프롬프트 업그레이드). 출력은 EXP02 스타일 파일명으로
data/AndroidControl_EXP04/ 에 stage1 train + test 6종 (총 7 파일) 생성.
**Stage 2 는 보류** — 이번 미러 대상이 아니다 (stage1 만).

EXP04 pool ⊆ EXP03 pool (멤버십 drift 0) 이므로, EXP01 구동이든 EXP03 출력 구동이든
결과 멤버십은 동일하다. EXP03 출력 대비 누락(drop): train 320, 전체 stage1 7파일 합 450.
...
- 본문(messages): EXP04 원천 레코드 (좌표 point 표현 + 업그레이드 프롬프트 유지).
- EXP04 원천에 키가 없는 EXP01 레코드는 제외한다 (EXP04 가 EXP01 대비 작음).
...
# (EXP01 입력 파일, EXP04 출력 파일, stage1 여부)
# stage1 train 은 ratio73(train_7_3) 만 미러 → EXP02 스타일 단일 train 명으로 출력.
# 나머지 test 는 EXP01 과 동일 파일명 (EXP02/EXP03 와도 byte 구조 동일).
...
def mirror(
    ...
    *,
    ...
    for rec in rows:
        ...
        e04 = idx.get(key_of(imgs))
        if e04 is None:  # EXP04 원천에 없는 데이터 → 제외
            dropped += 1
            continue
        out.append({**e04, "images": imgs})  # 본문=EXP04 좌표/프롬프트, 경로=EXP01
    return out, dropped
...
def main() -> int:
    ...
    out_dir = base / "AndroidControl_EXP04"
    out_dir.mkdir(parents=True, exist_ok=True)

    action_idx = build_index(
        src / "implicit-world-modeling_stage1_action_xy_prompt-enhanced.jsonl"
    )
    state_idx = build_index(
        src / "implicit-world-modeling_stage1_state_xy_prompt-enhanced.jsonl"
    )
    print(f"EXP04 index: action={len(action_idx)} state={len(state_idx)}")
