...
WEIGHT_MAP: dict[str, float] = {
    "ADDED": 2.0,  # 새로 생긴 element → 강조
    "MODIFIED": 2.0,  # 변경된 element    → 강조
    "UNCHANGED": 1.0,  # 동일한 element    → 기본 가중치
}
...
def get_element_char_spans(html: str) -> list[tuple[int, int, dict]]:
    ...
    for el in soup.find_all(True):
        ...
        spans.append((char_start, char_end, {"tag": tag, "index": idx, "text": text}))
...
def _make_el_key(el: dict) -> tuple:
    ...
    index 속성이 -1이 아니면 (tag, index)로 구분.
    index가 없는 경우 (tag, text) 조합으로 fallback.
    """
    if el["index"] != -1:
...
def build_token_weights(
    ...
    Returns:
        ...
        - assistant 구간 : UNCHANGED=1.0 / MODIFIED=2.0 / ADDED=3.0 (기본값)
          element에 해당하지 않는 토큰(태그 문법 등)은 1.0 유지
    """
    ...
    # ── 3. 기본 weight 배열 초기화 ─────────────────────────────────────────
    weights = [0.0] * n_prefix + [1.0] * n_asst
