...
> ⚠️ **함정 19 — 절단(1024) leaf 에는 산출하지 않는다.** 잘린 예측은 element 수가 줄고, 그러면 `copy_rate_pred` 가 **과소평가**된다 — 하필 이 지표가 재려는 방향으로 편향되어 실제보다 "덜 베낀 것처럼" 보인다. hungarian 계열이 그냥 무효인 것과 달리 **한쪽으로 틀린다.**
