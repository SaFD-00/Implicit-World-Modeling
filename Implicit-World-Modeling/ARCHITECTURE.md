...
| **AC_EXP06** | **절대 픽셀 840×1876** | Qwen2.5-VL 계열 (**관례** — 아래 각주) |

재확인: `python -c "from implicit_world_modeling.lf_registry import eligible_models as e; print(e('AndroidControl_EXP05'))"`

> ⓘ **AC_EXP06 은 EXP05 의 비증강 Stage-2 대조군**이며 `DATASET_MODEL_ELIGIBILITY` 에 **아직 미등록**이다 — Qwen2.5-VL/절대픽셀 자격은 **관례일 뿐 코드로 `exit 1` 강제되지 않는다** (merge 경로가 eligibility guard 를 거치지 않아 미등록 상태로 학습 성공; eval un-defer 시 등록 예정). 따라서 EXP06 행은 EXP05 와 달리 함정 2 의 코드 가드 대상이 아니다.
...
정본은 `lf_registry._LONG_CUTOFF_DS` (`EXP03/04/05` — `gen_configs` 학습 cutoff), 평가측 분기는 `_common.sh::build_infer_cmd` (`IWM-AC_EXP0{3,4,5}*` / `IWM-AC_EXP06*` prefix). **EXP06 은 `_LONG_CUTOFF_DS` 에 없다** (lf_registry 미등록) — 학습측 24576 은 hand-authored `configs/train/IWM-AC_EXP06/stage2_lora/*.yaml` 의 `cutoff_len` 에서, 평가측은 `build_infer_cmd` 의 `IWM-AC_EXP06*` 분기에서 온다.
