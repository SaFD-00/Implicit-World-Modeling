...
- **P2 — 해소, 그런데 반대 방향에도 같은 결함이 있었다**: `tests/test_diff_loss_double_ce.py` 에 `pytest.importorskip("llamafactory")` 를 넣어 `.venv` 에서 `--ignore` 없이 돌게 했다. 그 뒤 conda env 로 전체를 돌려보니 이번엔 **`tests/test_thought_eval.py` 의 `import sacrebleu` 하드 임포트로 스위트 전체가 collection ERROR** 였다(대칭 결함, 지금까지 아무도 conda 에서 `pytest tests` 를 통으로 돌린 적이 없었다는 뜻). optional import + `requires_sacrebleu` skipif 로 **BLEU 검증 6 건만** 골라 skip 하게 바꿔 나머지 커버리지를 지켰다. 결과: **`pytest tests` 가 두 env 모두 플래그 없이 그린** — conda **685 passed / 15 skipped**, `.venv` **681 passed / 10 skipped**. ⚠️ **어느 쪽 그린도 전량 검증이 아니다** (conda 는 BLEU 미검증, `.venv` 는 diff-loss 미검증). `.venv` 가 collection-skip 하는 `test_diff_loss_double_ce.py`(module 전체가 skip 1건으로 잡힘)는 conda 에서 실제로 돌려 **10 items 전량 통과** 확인했다 — `.venv` 스킵 10건과는 다른 셈이다(그중 9건은 `test_mirror_experiment.py` 의 무조건 skip 으로, conda 에서도 동일하게 skip 된다).
- **P3 — 해소 + 예상 밖의 소득(재현성 증명)**: 빌더를 `--seed 7 --revision 66285546…`(sidecar 의 `revision_resolved` 고정 — 기본값 `None` 은 Hub HEAD 재해석이라 토크나이저가 바뀌면 `token_weights` 가 조용히 달라진다) 로 재실행했다. 1.4 GB 백업 후 md5 대조 결과 **train 2 + test 6 + `dataset_info.json` 전부 바이트 동일**하고 **바뀐 것은 sidecar 하나뿐**이었다 — 구경로(`implicit-world-modeling_stage1_train.jsonl*`)가 신경로로 갱신되고 `revision_arg` 가 `null` → 고정 SHA 로 명시화됐다. 즉 P3 가 닫혔을 뿐 아니라 **같은 seed·revision·소스에서 1.06 GB 코퍼스가 bit-identical 하게 재생산된다**는 증명을 얻었다. 빌더 자체 검사도 재통과(50,000 = state 40,000 + down 10,000, `diff_totals` 동일, **누출 0**).
- **누출 불변식 독립 재검증**: 재빌드와 별개로, **배포돼 있던** train 산출물의 `(episode,step)` 키를 **복구된** EXP05 test 4 종 키(union **10,795** — 기록치와 일치)와 교차했다 → `stage1_train` 40,000 키 **누출 0**, `stage2_train` 14,950 키 **누출 0**(15,000 중 50 은 `home.jpg` open 증강이라 키가 없다 — `AUG_PER_STAGE=50` 과 정합). 07-25 심링크 사고가 test 키 집합을 바꿔놓지 않았음을 이걸로 확정했다. 산출물에서 키를 못 뽑는 image-stripped 10 k 는 이 검사의 사각이며, 그건 빌더의 **풀 단계 assert** 가 담당한다(파일이 아니라 실행에 있다).
- **발굴·수정 ① `--stage1-epoch` 가 EXP07 체크포인트를 지목할 수 없었다**: EXP07 stage1 은 `save_steps 0.25` 라 라벨이 `0.25/0.5/0.75/1` 인데, `_common.sh` 의 `parse_args`·`parse_eval_args` 가 `--stage1-epoch` 를 `^[0-9]+$` 로 검증해 **소수를 거절**했다. `resolve_stage1_adapter` 의 주석은 "분수 라벨도 그대로 매칭한다" 였고 [README:163](../Implicit-World-Modeling/README.md)·[ARCHITECTURE §4](../Implicit-World-Modeling/ARCHITECTURE.md#4-파이프라인-컴포넌트) 는 이미 "소수를 넘길 수 있다" 고 **약속하고 있었다** — 즉 신규 기능이 아니라 **코드가 문서화된 계약을 어기고 있던 것**이고, 그 결과 EXP07 의 핵심인 **merge X(`world-model-adapter`) 경로가 통째로 도달 불가**였다. 정규식을 `^[0-9]+(\.[0-9]+)?$` 로 완화(+ `--help` 문구). 검증: `0.25` 통과 → `..._world-model_from_adapter-ep0.25` 로 해석, `abc`·`0.25.1` 은 여전히 거절, **EXP05 정수 경로 불변**, eval 체인은 `SaFD-00/qwen2.5-vl-3b-ac-exp07-world-model-stage1-lora-epoch0.25-stage2-lora-adapter-epoch3` 까지 끝까지 조립됨.
- **발굴·수정 ② 심링크 자기치유가 원천 디렉토리까지 링크했다**: `_common.sh` 의 자기치유 루프가 `data/*/` 를 전부 링크해 **등록 키가 하나도 없는** `AndroidControl_EXP07_src` 까지 심링크를 만들었다(스크립트를 돌릴 때마다 git status 오염). `*_src` 를 제외하도록 고치고 잉여 심링크를 제거 — 커밋된 팜은 다시 **등록 데이터셋 10 개 정확히**.
- **채점 경로 실행 검증 (추론 없이 가능한 절반)**: EXP07 test 실데이터에서 GT 를 그대로 예측으로 넣은 오라클 300 쌍으로 채점기 3 종을 **실제로 돌렸다** — hungarian `--match-mode pos` f1/BLEU/ROUGE-L/EM **1.0**, action `--coord-mode xy` **parse 100 %** · step/macro/bbox/text 전부 **1.0**(stage1 action · stage2 공통), thought `cosine 1.0` · `missing 0 %`. myset 포맷이 채점기와 완전 호환됨을 파싱 추정이 아니라 실행으로 확정했다.
- **dry-run 관통**: `stage1_train`(full/lora) · `stage2_train`(base / world-model-lora / world-model-adapter) · `stage2_merge` 를 `DRY_RUN=1` 로 통과시켜 모델 자격 가드(`qwen3-vl-4b × EXP07` → exit 1) · dataset 등록 · gpu_policy(global batch 64) · deepspeed config · override 조립을 확인했다. 부수 확인: **CUDA/nvcc 가드는 conda env 활성화에 의존한다** — 가드의 기본값 `/usr/local/cuda` 는 이 머신에 **없고**, conda 가 `CUDA_HOME` 을 env prefix(nvcc 12.8 = torch cu12.8)로 세팅해서 통과한다.
- **⛔ 학습 미착수 — 로컬 GPU 없음**: RTX 5090 2 장을 **다른 사용자(`byeongung.cho`)의 sglang 서버 2 개**(PID 7318·7328, Qwen2.5-VL 3B/7B, 33 시간 가동)가 점유해 카드당 여유가 **3.2 / 3.1 GiB** 다. qwen2.5-vl-3b LoRA 도 못 올린다 — `cutoff_len 24576` × vocab 151936 의 lm_head logits 만 bf16 **~7.5 GiB** 이고 ZeRO-3·CPU offload 로 줄지 않으며, `SMOKE=1` 은 `max_samples`/`max_steps` 만 줄이고 cutoff 를 안 건드려 마찬가지다. **타인 프로세스는 건드리지 않았다.** 카드 확보(협의) 또는 원격이 선결 조건.
- 변경: `scripts/_common.sh` · `tests/test_diff_loss_double_ce.py` · `tests/test_thought_eval.py` · `Implicit-World-Modeling/{AGENTS,ARCHITECTURE}.md` · `docs/ROADMAP.md`. `data/AndroidControl_EXP07/stage1_train.jsonl.meta.json` 은 재빌드로 갱신됐으나 `data/` 는 비추적이다.
- 결과/검증: `pytest tests` conda **685 passed / 15 skipped** · `.venv` **681 passed / 10 skipped** · `ruff check` **All checks passed** / `ruff format --check` **32 files already formatted** · `bash -n scripts/*.sh` exit 0 · `gen_configs --check` **193 YAML 일치** · md5 대조 **8 파일 동일, sidecar 1 개만 변경** · 누출 교차검증 **0 / 0** · 오라클 채점 3 종 전 지표 **1.0**.
- 후속: **P5** conda env 에 `sacrebleu` 부재 → stage2 eval 의 `bleu` 가 `null` 로 빠진다. 설치는 반드시 `pip install --no-deps sacrebleu portalocker tabulate colorama` (평범한 `pip install` 은 numpy 를 올려 같은 env 의 torch/vLLM 을 깨뜨릴 수 있다). **P6** GPU 확보 — 그때까지 EXP07 학습은 대기. 실제 학습·추론 스모크(vLLM 경로)는 여전히 미실행이다.
- 카테고리: devlog

## 2026-07-25 — IWM: 데이터 파일명 규칙 확정 + 전면 rename(표현 서픽스 폐기) + dataset_info 사문키 10개 제거(51→49) + 문서 7종 현행화

파일명이 소속·표현·필터 여부를 중복해서 말하던 관례(`implicit-world-modeling_stage1_action_xy_pixel-aligned.jsonl`)를 걷어내고 **"디렉터리가 실험을 말하면 파일은 침묵한다"** 를 규칙으로 확정했다. 규칙 자체를 [ARCHITECTURE §3 「데이터 파일명 규칙」](../Implicit-World-Modeling/ARCHITECTURE.md#3-데이터와-설정-계약)에 명문화하고 [AGENTS 하드 제약 17](../Implicit-World-Modeling/AGENTS.md)로 걸었다.

- **명명규칙 4 항 — 접두는 "공유 폴더에서만", 표현 서픽스는 폐기**: ① EXP 디렉터리(`AndroidControl_EXP{NN}/`·`MonkeyCollection/`·`MobiBench/`) 안은 `stage{N}_<role>.jsonl` 로 침묵한다(`implicit-world-modeling_` 접두 제거). ② 여러 실험군이 공유하는 `data/AndroidControl/` 의 변형본만 `EXP{NN}_` 접두를 쓴다 — `_xy`/`_pixel-aligned`/`_filtered` 같은 **표현 서픽스는 EXP 소속이 이미 함의**하므로 제거했다(`*_xy.jsonl`→`EXP03_*`, `*_xy_pixel-aligned.jsonl`→`EXP05_*`, `*_filtered.jsonl`→`EXP01_*`; `*_xy_prompt-enhanced`→`EXP04_*` 는 **디스크 부재라 실제 mv 없이 문서상 기대명만 갱신**했다 — EXP04 가 재빌드 불가인 이유가 그 부재다). ③ 멀티-exp 공유 파일은 **직접 소비하는 최초 EXP 하나**만 소유하고 재사용 관계는 문서가 기록한다(태생부터 공동 소유면 `EXP05+07_`). ④ `_without_open_app`·sidecar(`<train>.jsonl.meta.json`) 규칙과 dataset key 이름은 **전부 불변**이고, 원본 3 종(`implicit-world-modeling_*`)·`episodes_meta.jsonl`·`images/` 는 어느 EXP 소속도 아니라 이름을 바꾸지 않았다.
- **dataset_info 사문키 10개 제거 — 51 → 49 키**: 과거 스키마의 `IWM-AC_stage*` 6 개와 `IWM-AC_2_*` 4 개는 실파일이 없는 등록이었다(`require_dataset_registered` 가 키 존재만 봐서 무해했지만 오용 유인이었다). 같은 커밋 범위에서 EXP07 8 키가 추가돼 순증감은 −2 다. **이제 전 키가 실파일로 resolve 한다.**
- **EXP05 중복본 4개 삭제**: `stage1_0711_*` 4 개를 `EXP05_*` 와 md5 동일성 확인 후 삭제했다(**실행 주체 보고치** — `data/` 가 비추적이라 사후로는 *부재*만 재확인 가능하고 바이트 동일성은 재검증 불가).
- **문서 7종 현행화 — EXP07 서술이 계보째로 stale 이었다**: rename 반영 과정에서 문서의 EXP07 서술이 **재빌드 이전 잠정본**(EXP05 stage1 서브샘플 30K + EXP05 심링크 4 + `dataset_info` EXP05 포인터 5)을 기술하고 있음이 드러났다. 실제 현행은 별도 0725 소스(`data/AndroidControl_EXP07_src/`)에서 자체 빌드한 **train 2(stage1 50K/stage2 15K) + 자체 test 6 종**이고 **8 키 전부 자체 경로·심링크 0** 이다. 계보 화살표(`AC_EXP05 └── AC_EXP07`)를 끊어 **EXP01 계보 밖 형제**로 고치고, EXP05 의존이 *test 키 정의뿐*(누출 0 을 위해 EXP05 test 의 `(episode,step)` 키를 재현·제외)이라는 점을 명시했다. `--links-only` 플래그는 이미 사라졌으므로 해당 서술도 삭제했다.
- **동봉 발견 — `_DUAL_TASK_TEST` 에 EXP07 이 없다**: EXP07 은 dual-task test 4 종이 디스크·`dataset_info` 에 다 있는데 `lf_registry._DUAL_TASK_TEST` 에는 빠져 있어 `build_configs()` 가 else 분기를 타고 **미등록 키** `IWM-AC_EXP07_stage1_test_{id,ood}` 를 `ds_s1_test_*` 에 담는다. **학습·평가는 무증상**이다(셸이 `${eval_prefix}_stage1_test_{id,ood}_${task}` 로 이름을 직접 조립하고, stage1 YAML 은 train 만 참조한다). **그러나 노트북의 등록 검증 셀 `_expected_keys()` 는 같은 분기를 미러해 이 필드를 실제로 읽으므로**, EXP07 에 대해 없는 bare 키를 기대하고 실재하는 4 키를 누락으로 본다 — 그 셀의 EXP07 판정은 신뢰할 수 없다. 정본 수정은 `_DUAL_TASK_TEST` 에 `"AndroidControl_EXP07"` 추가이며, 코드 수정은 이번 범위 밖이라 ARCHITECTURE §3 에 함정으로 남겼다.
- 변경: 데이터 rename 은 `data/` (gitignore 대상) 에서 수행 — 추적 대상 변경은 `configs/lf_dataset/dataset_info.json` · `implicit_world_modeling/{lf_registry,gen_configs}.py` · `scripts/{filter_long_samples,split_data,mirror_experiment,build_exp05_data,build_exp05_stage2_data,_common.sh,…}` · `tests/*` 이고, 문서는 `Implicit-World-Modeling/{README,ARCHITECTURE,AGENTS}.md` · 루트 `README.md` · `docs/{ARCHITECTURE,ROADMAP,DEVLOG}.md` 7 종이다. (mv 81 건 · 참조 동기 20 파일은 rename 실행 주체의 보고치이며 `data/` 가 비추적이라 사후 재검증 불가.)
- 커밋: 없음 (기록 시점 uncommitted).
- 결과/검증: `.venv/bin/python -m pytest tests --ignore=tests/test_diff_loss_double_ce.py` → **681 passed / 9 skipped / exit 0**. ⚠️ **`.venv` 에서는 `pytest tests` 전체가 collection error 로 중단된다** — `tests/test_diff_loss_double_ce.py` 가 `llamafactory.extras.constants` 를 모듈 레벨에서 hard-import 하고 그 체인이 `peft` 를 요구하는데 둘 다 `.venv` 에 없다(skip 가드 없음, HEAD 부터 그랬다). 그 파일을 포함한 그린은 학습 env(conda)에서만 재현된다. 그 외: `dataset_info` 키 51→49 및 제거된 10 키 = `IWM-AC_stage*` 6 + `IWM-AC_2_*` 4 를 HEAD 대비 diff 로 확인 · EXP07 8 키 `file_name` 전부 `../../data/AndroidControl_EXP07/…` · `data/AndroidControl_EXP07/` 는 실파일 9 개(심링크 0) · `data/AndroidControl/` 에 `stage1_0711_*` 부재 · 문서 구명 잔존 grep 은 과거-사건 서술 4 건만 남음(Drive 폴더명 `'0711_버젼'` 인용 2 · 0710/0711 좌표 버그 기록 2).
- 후속: **P1** ~~`_DUAL_TASK_TEST` 에 EXP07 편입~~ → **같은 날 해소**(편입 완료, `gen_configs --check` 193 YAML 무변경·pytest 그린). **P2** `tests/test_diff_loss_double_ce.py` 에 `pytest.importorskip` 가드를 넣어 `.venv` 에서도 `pytest tests` 가 한 줄로 돌게 할지 결정. **P3** `data/AndroidControl_EXP07/stage1_train.jsonl.meta.json` 의 `input`/`output` 필드가 rename 이전 경로(`implicit-world-modeling_stage1_train.jsonl`)를 그대로 담고 있다 — 손으로 고치지 말고 빌더 재실행으로 재생성해야 한다. **P4** ~~`ANSTERM_PER_STAGE` 데드 상수·docstring 불일치~~ → **같은 날 해소**(상수 삭제, docstring 을 동적 규칙+실현치 태깅으로 정정).
- 카테고리: devlog
...
직전 F2 마일스톤이 musicplayer 1쌍뿐이라 single-case 였던 것을 **두 번째 앱(osmand)** 에서 이진 질문 하나로 검증하고, 측정 도구 결함(S-3)을 고쳐 실전 검증했으며, `AGENTS.md` 에 평가 방법·불변식 9항을 계약화했다. 상세는 [분석 보고](../.claude/analysis/2026-07-14_15-30-41_s1-f2-osmand/README.md) · [revise 기록](../.claude/devlog/2026-07-14_15-30-41_s1-osmand-and-docs.md) · [사전등록](../.claude/handoff/s1-prereg.md).
...
리셋 프로토콜 paired-arm ablation(pre-fix `6ff8e95` vs fix `fe12f46`, musicplayer+calendar, 900s × 4 run + 동일 코드 반복측정 1 run)으로 budget-loop fix(D1/D2/D3)가 실제로 무엇을 했는지 판정했다. 결론은 **fix의 다양성 이득이 노이즈와 구별되지 않는다**는 것이고, 그 과정에서 진짜 병목이 따로 드러났다. 상세는 [분석 보고](../.claude/analysis/2026-07-14_04-05-29_iter6-controlled-ablation/README.md) · [revise 기록](../.claude/devlog/2026-07-14_04-05-29_iter6-d3-recalibration.md).
...
- 변경: `Monkey-Collector/src/monkey_collector/config.py` · `pipeline/collector.py` · `config/run.yaml` · `tests/unit/test_config.py`(D3 상수 동기화 지점 7곳) + `Monkey-Collector/AGENTS.md` · `.claude/handoff/HANDOFF.md`. 문서 정합: `Monkey-Collector/ARCHITECTURE.md` §7 config 표(D3 행 신설, `data_dir`/`runtime_dir` CWD-상대 명시).
