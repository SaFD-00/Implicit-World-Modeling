...
- `screen_matching.{cluster_merge_tolerance, max_expand_iters, luminance_prefilter, luminance_threshold, screenshot_diff_threshold, luminance_low_res_width}`
...
- `--luminance-prefilter`: Stage-0 luminance prefilter `on` / `off`. 기본 `on` (`--element-extraction on` 필요). 픽셀상 동일한 재방문 화면을 LLM 0회로 단락
...
│   │   └── 0000/                          # observation (동일 page 의 서로 다른 시각 상태)
...
이미 저장된 observation 과 픽셀상 동일하거나(luminance prefilter) 구조적으로 동일한(structural exact-match) 화면을 재방문하면 **새 파일을 전혀 쓰지 않는다** — 기존 `screen_matcher` 가 어떤 observation 과 같은지 판단해 재사용 여부(`is_new_observation`)를 결정한다.

`events.jsonl` 은 처리된 화면마다(로딩/스플래시로 거부된 화면 제외) 한 줄이며, 각 줄은 `frame_index`(정렬용 단조 증가 카운터) 와 `page_key`/`observation_num`(그 화면이 실제로 위치한 `data/{package}/pages/{page_key}/{observation_num:04d}/` 를 가리키는 조인 키) 을 담는다 — world-modeling converter 와 offline page-graph 재빌드가 이 조인 키를 쓴다. `step` 은 루프 카운터 라벨로 정상 action 에서만 증가한다. 자세한 스키마는 [ARCHITECTURE.md](./ARCHITECTURE.md) 참조.

`elements.json`(`--element-extraction on` 일 때만) 은 `ScreenMatcher` 의 element-set match 를 담는다: `page_key`/`match_type`/`is_new_page`/`activity` + element family(`name`/`description`/`parameters`/`element_index`/`key_element_index`, index 는 그 observation 의 `encoded.xml` 기준). 새 페이지면 그 화면에서 추출된 family 가, merge·재방문(`is_new_page=false`)이면 매칭된 page 의 저장 element 를 현재 화면 anchor 에 재매핑해 채운다. interactable 없는 로딩/스플래시 화면은 page 로 등록하지 않으므로(첫 유효 화면이 `page_0`) 관측 자체가 생성되지 않는다. LLM 호출 결과라 `regenerate` 로는 재생성되지 않는다.
...
│   └── integration/                  # 7 files
│
└── (gitignored) data/, runtime/, logs/, *.egg-info/
```
