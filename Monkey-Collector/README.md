...
- input text 생성은 OpenAI Responses API (`gpt-5-nano`) 기반 `api` 모드와 hardcoded `random` 모드를 지원한다. `OPENAI_API_KEY` 가 없으면 자동으로 random fallback 한다.
...
주요 런타임 의존성: `loguru`, `Pillow`, `openai`, `python-dotenv`, `pyvis`, `requests`, `gplaydl`. 정확한 버전은 [`pyproject.toml`](./pyproject.toml) 의 `[project] dependencies` 섹션을 참조한다.
