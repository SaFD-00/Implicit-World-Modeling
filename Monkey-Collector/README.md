...
- LLM 사용처는 두 곳이지만 **기본값은 (1) 입력 텍스트 생성 전용**이다(element 추출은 opt-in). 둘 다 하나의 **OpenRouter Chat Completions** 공용 클라이언트(기본 모델 `qwen/qwen3.7-plus`)를 쓴다: (1) **input text 생성** (`--input-mode api`, 없으면 hardcoded `random`) — 프롬프트에 현재 탐색 중인 앱 설명(`catalog/apps.csv` 의 `app_name (category/sub_category) — notes`, csv 미등록 앱은 package_id 로 폴백)을 함께 넣어 앱 도메인에 맞는 입력값(쇼핑앱 검색창→상품 검색어, 메모앱→메모 내용 등)을 생성한다, (2) **element 추출 + element-set screen matching** (기본 **off**, opt-in: `--element-extraction on` / `element_extraction: true`, MobileGPT-V2 Node-Clustering 포팅): 화면마다 element 를 한 번의 호출로 추출(각 element 에 `element_index`(같은 기능 family)와 `key_element_index`(대표 anchor)를 함께)하고, 그 element-set 으로 page 를 식별한다 — 이 page 식별이 page_graph 와 탐색 abstract page 를 모두 결정하고 결과는 그 observation 의 `elements.json` 으로 저장된다. `OPENROUTER_API_KEY` 가 없으면 자동 비활성화된다(입력은 random fallback, page 식별은 구조 지문으로 degrade). **Stage-0 luminance prefilter**(MobileGPT-V2 `luminance_prefilter` 포팅, 기본 ON)는 `element_extraction off` 여도 prefilter-only ScreenMatcher(구조 지문 + luminance/observation dedup, LLM 0회)로 계속 동작한다 — screenshot 의 luminance 지문(순수 PIL, BT.601 luma)을 저장 page 들과 비교해 픽셀이 사실상 동일한 재방문 화면은 LLM 호출 없이 그 page 로 단락한다(시각적으로 동일한 화면 재방문 시 element 추출 비용 절감). 이렇게 필터된 재방문의 **저장 여부**는 `screen_matching.persist_filtered`(기본 ON)가 결정한다 — 기본값에선 필터된 재방문도 그 page 아래 **자체 observation**(방문마다 `0,1,2,…` 체인)으로 저장하고, off 로 두면 재사용 관측은 파일을 전혀 쓰지 않는다(기존 절약 동작).
...
- `--element-extraction`: element 추출 + element-set screen matching `on` / `off`. 기본 `off` — 기본적으로 LLM 은 입력 텍스트 생성에만 쓰고 element 추출은 opt-in(`on` 이어도 LLM 클라이언트 없으면 자동 비활성, 구조 지문 식별로 degrade). `on` 이면 화면마다 element(family + anchor)를 추출해 page 식별에 쓰고 `xml/{step}_elements.json` 으로 저장. (`--screen-grouping` 은 deprecated alias)
...
- `--luminance-prefilter`: Stage-0 luminance prefilter `on` / `off`. 기본 `on` (`--element-extraction off` 여도 prefilter-only ScreenMatcher 로 동작 — 더는 element_extraction 을 요구하지 않음). 픽셀상 동일한 재방문 화면을 LLM 0회로 단락
- `--luminance-threshold`: 픽셀 밝기 차이 `|ΔY|` 임계값 0–255 (기본 10)
- `--screenshot-diff-threshold`: 같은 page 로 볼 차이 픽셀 비율 (기본 0.02)
...
`elements.json`(`--element-extraction on` 일 때만) 은 `ScreenMatcher` 의 element-set match 를 담는다: `page_key`/`match_type`/`is_new_page`/`activity` + element family(`name`/`description`/`parameters`/`element_index`/`key_element_index`, index 는 그 observation 의 `encoded.xml` 기준). 새 페이지면 그 화면에서 추출된 family 가, merge·재방문(`is_new_page=false`)이면 매칭된 page 의 저장 element 를 현재 화면 anchor 에 재매핑해 채운다. interactable 없는 로딩/스플래시 화면은 page 로 등록하지 않으므로(첫 유효 화면의 page_key 가 `0`) 관측 자체가 생성되지 않는다. LLM 호출 결과라 `regenerate` 로는 재생성되지 않는다.
...
│   │     ├── screen_matching/  (screen_matcher, ui_attributes, set_classifier, page_knowledge, rehydrate)
