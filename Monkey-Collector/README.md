...
- 세션은 두 root 로 나뉜다 — `data/raw/{package}/`(영속: pages/observations, page_graph), `runtime/{package}/`(휘발성: metadata, events, cost/coverage).
...
- 큐 구성 시 `runtime/{pkg}/metadata.json` 의 `completed_at` 이 채워진 앱은 **완료로 판정되어 스킵**. `--force` 로 우회하거나, 중단된(미완료) 세션은 `completed_at` 이 `null` 이라 자동으로 resume 된다(재개 시 `data/raw/{pkg}/pages/` 로부터 page 지식도 함께 복원).
...
- `--data-dir` / `--runtime-dir`: 두 root (기본 `data/raw`/`runtime`)
