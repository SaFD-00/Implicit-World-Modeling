...
- LLM 사용처는 두 곳이지만 **기본값은 (1) 입력 텍스트 생성 전용**이다(element 추출은 opt-in).
  ...
  - 이 추출은 수집 루프의 hot path 위에서 돌기 때문에 `max_tokens`/`timeout`/`max_retries` 로 bound 되어 runaway 생성이나 hung provider 가 메인 루프를 블록하지 못하며, 실패(truncation/timeout 포함)는 빈 결과로 degrade 한다(값·근거는 [ARCHITECTURE.md](./ARCHITECTURE.md) 참조).
...
`elements.json` 은 `--element-extraction on` 일 때 `ScreenMatcher` 가 만든 match annotation(`page_key`/`match_type`/element family)을 담는다 — 스키마는 [ARCHITECTURE.md](./ARCHITECTURE.md) §5 참조.
