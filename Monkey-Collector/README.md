...
- 세션은 두 root 로 나뉜다 — `data/raw/{package}/`(영속: pages/observations, page_graph), `runtime/apps/{package}/`(휘발성: metadata, events, cost/coverage). `runtime/` 아래는 `apps/`(앱별 상태)와 `logs/`(실행 로그) 두 sub-root 로 정리돼 있다.
...
- 큐 구성 시 `runtime/apps/{pkg}/metadata.json` 의 `completed_at` 이 채워진 앱은 **완료로 판정되어 스킵**. `--force` 로 우회하거나, 중단된(미완료) 세션은 `completed_at` 이 `null` 이라 자동으로 resume 된다(재개 시 `data/raw/{pkg}/pages/` 로부터 page 지식도 함께 복원).
...
- `--all`: `--data-dir`/`--runtime-dir` 전체를 삭제 (다른 스코프 플래그와 상호 배타). 두 root 를 함께 지워야 한다 — `data/raw/` 만 남으면 다음 실행이 그 페이지 지식을 다시 rehydrate 해버려 "새 세션"이 되지 않는다.
  - reset 이 지우는 것은 **수집 세션 상태뿐**이다. 파생 학습 코퍼스 `data/processed/` 는 리셋 루트 `data/raw` 의 형제라 어떤 스코프도 닿지 않는다.
- `--apps`: 삭제할 package_id 리스트
- `--data-dir`: 수집 원본 루트 (기본 `data/raw`)
...
- `--data-dir` / `--runtime-dir`: 두 root (기본 `data/raw`/`runtime`). 세션은 `{data-dir}/{package}/` 와 `{runtime-dir}/apps/{package}/` 에서 찾는다
- `--package`: 변환할 세션의 package id (= `{data-dir}/` 와 `{runtime-dir}/apps/` 아래 디렉터리명)
- `--label`: 이미지 파일명에 사용되는 세션 라벨 정수 (기본 `1`).

`pages/` 가 있는 세션은 `events.jsonl` 의 `page_key`/`observation_num` 으로 정확히 조인한다. `pages/` 없이 구형 flat `xml/` 만 있는 세션(마이그레이션 이전 백업)은 자동으로 legacy 변환으로 degrade 되므로, 그런 백업은 `events.jsonl` 이 세션 디렉터리 안에 함께 있으므로, 세션들이 `{runtime-dir}/apps/` 아래에 오도록 지정한다 — 즉 백업 세션들을 담은 디렉터리 이름이 `apps` 여야 하고 `--data-dir` 은 그 `apps` 디렉터리를, `--runtime-dir` 은 그 부모를 가리킨다.

### `convert-all`

```bash
# 네 인자 모두 optional — 아래 값이 그대로 기본값이다
...
`--data-dir` 아래 각 세션 디렉터리명으로 `--runtime-dir` **아래 `apps/`** 의 대응 세션을 찾아 짝짓는다(`--runtime-dir` 은 root 이지 apps sub-root 가 아니다). 존재하지 않는 `--data-dir` 은 크래시 없이 경고 후 0건으로 종료한다. 세션으로 인정되는 것은 `pages/` 또는 구형 `xml/` 를 가진 하위 디렉터리뿐이라, `data/processed` 는 `data/raw` 의 형제이자 그 조건도 만족하지 않아 열거되지 않는다.
...
코퍼스는 `data/` 아래에서 이원화돼 있다 — `data/raw/` 는 **수집 원본**(page/observation 지식), `data/processed/` 는 `convert-all` 이 만드는 **학습 변환 산출물**(`gui-model_stage1.jsonl` + `images/`)이다. 수집기는 `data/raw` 에만 쓴다.

세션 자체는 두 root 로 분리된다 — `data/raw/` 는 영속 코퍼스(page/observation 지식), `runtime/` 는 세션 진행 상태:
...
├── apps/{package}/                        # 재개 상태·타임라인
│   ├── metadata.json
│   ├── events.jsonl
│   ├── activity_coverage.csv              # ground truth: catalog/activities.json (fallback: dumpsys)
│   └── cost.csv
└── logs/run_*.log                         # `run` 이 남기는 per-run 로그

data/processed/                            # 파생 — convert-all 산출물 (수집 세션 상태 아님)
...
세션 재개(resume) 시 `data/raw/{package}/pages/` 로부터 `ScreenMatcher` 지식과 `page_graph.json` 을 모두 복원한다(기존 page 를 "새 page" 로 재발견하지 않음) — 메커니즘은 [ARCHITECTURE.md](./ARCHITECTURE.md) §4~§5 참조.
...
│
└── (gitignored) data/, runtime/ (apps/·logs/ 포함), *.egg-info/
```
