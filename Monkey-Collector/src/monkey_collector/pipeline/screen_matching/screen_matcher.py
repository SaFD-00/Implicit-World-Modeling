...
the de-facto defence, which is why the guard covers the WHOLE merge path rather
than any single candidate class. The comparison is at PACKAGE
...
class ScreenMatcher:
    ...
    def __init__(
        ...
        # defence, so the guard covers the WHOLE BM25 merge path.
    ...
    def match(
        ...
        if not lines or len(self._registry) == 0:
            return self._new_page(fp_key, lines, current_feat, activity)

        # 2-3. BM25 top-K retrieval → same-package guard, then the conjunctive
        # (element AND pixel) verification.
        cur_set = set(lines)
        for page_key, _score in self._bm25.top_k(lines, self._bm25_top_k):
            page = self._registry.get(page_key)
            if page is None:
                continue
            package_ok, element_ok, pixel_ok = self._verify_candidate(
                cur_set, page, current_feat, activity,
            )
            if not package_ok:
                logger.debug(
    ...
    def _verify_candidate(
        ...
        """Run the full gate for one candidate → (package, element, pixel).

        The single place a merge is decided, so ``match()`` and the offline
        replay's diagnosis cannot drift apart.

        The PACKAGE guard comes first and covers EVERY merge: the candidate
        page's minting package must equal the current screen's, or the two
        screens are different apps and no element/pixel similarity may merge
        them. It ABSTAINS (passes) when either side's package is unknown —
        fail-open, so a missing activity label fragments nothing.
        """
        if not self._package_ok(page, cur_activity):
        ...
        return True, element_ok, pixel_ok
