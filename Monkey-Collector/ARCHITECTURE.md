...
- `CollectorService.kt`
  ...
  - **서버 `CAPTURE` poke 응답 (`handleCaptureRequest`)** — a11y 이벤트 없이도 서버가 프레임을 당겨갈 수 있는 pull 경로. 현재 XML 을 덤프해 마지막으로 보낸 프레임의 XML 해시(`TcpClient.lastSentXmlHash`)와 비교하고, **바뀌었으면 프레임(`S`+`X`), 같으면 `N`, 타깃 이탈이면 `E`** 로 답한다. 수집 중이 아니거나 · TCP 미연결 · 캡처가 이미 진행 중이거나 · 직전 프레임 전송 후 `MIN_CAPTURE_INTERVAL_MS`(3s) 가 안 지났으면 poke 를 무시한다. 캡처 본체(`captureAndSendFrame`)는 a11y 이벤트 경로와 공유한다 (프로토콜은 §3 참조)
...
- `TcpClient.kt`
  - App -> Server `P/S/X/E/N/F`. 한 캡처의 `S`(screenshot)+`X`(XML) 는 `sendFrame()` 이 **단일 `synchronized(writeLock)` 구간에서 연달아 쓰는 원자 쌍**이다 — 개별 send 만 잠그면 동시 캡처(레거시 이벤트 경로 ↔ poke 경로)가 `S(A) S(B) X(B) X(A)` 로 크로스페어링될 수 있어 프레임 단위로 원자화했다. 비원자 경로의 재도입을 막으려 `sendScreenshot`/`sendXml` 은 제거했다.
...
한 캡처의 `S`+`X` 는 항상 **원자 쌍**으로 나간다 (`TcpClient.sendFrame()` 의 단일 `writeLock` 구간 — 동시 캡처가 두 프레임을 교차시키지 못한다). `S`/`X`/`N`/`E` 는 **두 경로가 공유**한다 — a11y 이벤트가 뜬 push 경로와, 서버 `CAPTURE` poke 에 답하는 pull 경로다. poke 를 받은 클라이언트는 XML 이 바뀌었으면 프레임(`S`+`X`), 같으면 `N`, 타깃 이탈이면 `E` 로 답한다.
