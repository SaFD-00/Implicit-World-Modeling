...
대상 (현재): AC_3 (AndroidControl_3) Stage 1 두 source
(``gui-model_stage1_{state,action}_pred.jsonl``) + Stage 2 source
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AC_3 source jsonl 에서 mm-expanded length > threshold 샘플을 제거.",
    )
    ...
    sources = [
        ds_dir / "gui-model_stage1_state_pred.jsonl",
        ds_dir / "gui-model_stage1_action_pred.jsonl",
        ds_dir / "gui-model_stage2.jsonl",
    ]
    for p in sources:
        if not p.exists():
