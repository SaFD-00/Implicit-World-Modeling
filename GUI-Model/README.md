...
### 2.5) AndroidControl_3 준비 (state_pred + action_pred ratio mix, Stage 1 전용)
...
ratio 별로 **별개의 학습 가중치** 가 산출되므로 train/merge/eval 모두 `--dataset AC_3` 한 번에 ratio 3 종을 sweep 한다 (`--ac3-ratios r55,r73` 로 부분 실행 가능). 평가는 ratio 단일 (`--ac3-ratio r55` 기본) 로 고정 + state/action **두 task 를 각각 채점** (`_hungarian_eval.py` / `_action_eval.py`).
...
- **Stage 1 (AC_3)**: state_pred (random) + action_pred (action-type stratified) 를 비율 (`--ac3-ratios`, 기본 `3_7,5_5,7_3`) 로 혼합. id/ood 는 task 별로 분리된 4 파일.
- **AC_2 / MB**: split 불필요.
...
# Stage 1 — AC_3 학습 (state_pred + action_pred ratio 3 종 자동 sweep, Stage 1 전용)
...
```
GUI-Model/outputs/{AC|AC_2|AC_3_r37|AC_3_r55|AC_3_r73|MC}/
├── adapters/
