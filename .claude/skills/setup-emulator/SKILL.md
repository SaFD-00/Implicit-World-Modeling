...
> **⚠️ 동명(同名) 스킬 주의 — MobileGPT-V2 의 `setup-emulator` 와의 관계**: 옆 프로젝트 MobileGPT-V2 에도 같은 이름의 `setup-emulator` 스킬이 있다(`~/Desktop/Projects/MobileGPT-V2/.claude/skills/setup-emulator`). **본 문서에서 "MobileGPT-V2 의 setup-emulator" 라고 명시한 곳은 전부 그 옆 프로젝트 스킬**을 가리키며, 수식 없는 언급은 이 스킬(Monkey-Collector) 자신이다. 스킬은 프로젝트 스코프라 호출 충돌은 없다.
> 구조(AVD 생성 §0-a / 부팅 §1 / 설치 / 접근성 / **더미데이터 시드** / 검증 / Troubleshooting)와 **더미데이터 시드 레시피**(연락처·일정·노트·할일·음악 등)를 그쪽에서 차용하되, 대상이 다르다 — DroidTask/android_world 벤치마크가 아니라 **`catalog/apps.csv` 의 앱들**을, MobileGPT client 2종이 아니라 **Monkey-Collector client 1종(`com.monkey.collector`)** 을 다룬다. AVD `Pixel6-2` 는 MobileGPT-V2 의 `setup-emulator` 가 만든 Pixel6/API33 AVD 와 동일 스펙이며, 이미 존재하면 재사용한다. server IP 주입은 `update-client-ip` 와, 기본값 복원은 `reset-defaults` 와 메커니즘을 공유한다.
...
`Pixel6-2` 를 canonical 스펙(device `pixel_6`, image `$SYS_IMG`)으로 보장한다. 기본은 **없을 때만 생성**, `--recreate` 면 삭제 후 재생성(⚠️ 데이터 전소). 이미 MobileGPT-V2 의 `setup-emulator` 로 만들어 둔 동명 AVD 가 있으면 그대로 쓴다.
...
> device `pixel_6`(1080×2400) / **API 33(google_apis)** 은 MobileGPT-V2 의 `setup-emulator` canonical 과 동일하며, `google_apis` 이미지는 `adb root`/`run-as` 가 가능해 §6-b prefs 기록·폴백에 유리하다.
...
grep -q '^OPENROUTER_API_KEY=' .env 2>/dev/null \
  ...
  || echo "INFO: OPENROUTER_API_KEY 미설정 — 수집은 동작하나 LLM 입력생성(--input-mode api) 비활성"
```
...
# 노드 1개당 1줄로 쪼갠 뒤(sed), Button 클래스 + 정확일치 text 로 좁혀 bounds 를 뽑는다
...
SAVE=$(dumpui | grep 'class="android.widget.Button"' | grep 'text="SAVE &amp; READY"' | bounds_of)
if [ -n "$SAVE" ]; then read -r x1 y1 x2 y2 <<< "$SAVE"; adb -s "$SERIAL" shell input tap $(( (x1+x2)/2 )) $(( (y1+y2)/2 )); sleep 2; fi
# 시스템 "Start now" 다이얼로그가 뜨면 탭(appop 자동승인 시 안 뜸):
ST=$(dumpui | grep -E 'text="(Start now|시작)"' | bounds_of)
if [ -n "$ST" ]; then read -r x1 y1 x2 y2 <<< "$ST"; adb -s "$SERIAL" shell input tap $(( (x1+x2)/2 )) $(( (y1+y2)/2 )); fi
adb -s "$SERIAL" shell input keyevent KEYCODE_HOME
```

> **⚠️ 위 3줄은 전부 실측으로 깨졌던 지점이라 그대로 유지할 것**(2026-07-20 Pixel6-2 수정):
> 1. **`tr '>' '>\n'` 은 no-op** — `tr` 은 문자 1:1 치환이라 한 문자를 두 문자로 바꿀 수 없다(set2 가 set1 길이로 잘려 `>`→`>`). XML 이 **한 줄로 남고**, 이어지는 greedy `sed 's/.*bounds=…'` 가 그 줄의 **마지막** bounds 인 `navigationBarBackground`(`[0,2361][1080,2424]`)를 잡는다 → 내비게이션 바를 탭해 **런처로 튕긴다**. 반드시 `sed 's/></>\n</g'` 로 노드를 줄분리한다.
> 2. **`grep -i` 금지** — 상태 라벨 `"Status: Configure server and tap Save & Ready"` 가 버튼보다 먼저 매치돼 TextView 를 탭하게 된다. **대소문자 구분 + `class="android.widget.Button"` 한정**이 필요하다.
> 3. **`set -- $VAR` 는 zsh 에서 분리되지 않는다** — zsh 는 unquoted 파라미터 확장을 word-split 하지 않아 `$1` 에 `"0 2361 1080 2424"` 가 통째로 들어가고 `bad math expression` 으로 죽는다. `read -r x1 y1 x2 y2 <<< "$VAR"` 를 쓴다(bash/zsh 공통).
...
explorer 가 리스트/상세/편집 화면을 탐색할 콘텐츠가 있도록, 설치된 카탈로그 앱에 소량(앱당 3~5건)의 더미데이터를 넣는다. MobileGPT-V2 의 `setup-emulator` 시드 레시피를 재사용하며 **모두 멱등(앱별 sentinel)** 이다. 앱별 명령·스키마·gotcha 는:
...
> 시드 레시피는 MobileGPT-V2 의 `setup-emulator` 에서 Pixel6/API36 으로 실삽입·UI 검증된 것을 가져왔고, **Pixel6-2/API33 에서도 동일하게 동작 재확인**(연락처 5·일정 4·Markor 4·org.tasks 4·RetroMusic 3·OpenTracks 3·Joplin 3). RetroMusic 음악은 호스트 `ffmpeg` 필요(없으면 그 앱만 skip). Simple Notes·Simple Clock·SMS·Broccoli·Expense 는 현재 MC 카탈로그 미포함이라 자연 skip.
...
> **다음 단계**: 셋업 후 `uv run monkey-collect run --apps all` 으로 전체 수집을 시작한다. LLM 입력생성을 쓰려면 `.env` 에 `OPENROUTER_API_KEY` 를 채우고 `--input-mode api` 를 준다.
>
> **⚠️ 기본 예산은 스텝이 아니라 시간이다 — `--apps all` 전에 반드시 확인**: `config/run.yaml` 의 canonical 기본값은 `budget_mode: time` + `max_duration: 2h` 로 **앱당 2시간**이다(`max_steps: 1500` 은 `budget_mode=steps` 일 때만 쓰인다). installed=true 가 30~50개면 **총 60~100시간**이 되므로, 전수 수집 전에 `--duration 10m` 같은 현실적 캡을 주거나 config 를 낮춘다. `--steps N` 을 주면 `budget_mode` 가 steps 로 추론된다. 실측 처리량(Pixel6-2/API33): 약 **10~22 스텝/분**(앱 복잡도에 따라). `max_steps_without_new_page: 98` 덕에 탐색이 포화된 앱은 캡보다 일찍 자체 종료된다.
>
> **⚠️ `--apps` 에 여러 패키지를 변수로 넘길 때 zsh 주의**: `TARGETS=$(cat list.txt); monkey-collect run --apps $TARGETS` 는 zsh 에서 **word-split 되지 않아** 전체 문자열이 **패키지 1개**로 전달된다 → 그 문자열이 그대로 세션 디렉터리명이 되어 `OSError: [Errno 63] File name too long` 으로 죽는다(2026-07-20 실측). `bash -c 'T=$(cat list.txt); exec uv run monkey-collect run --apps $T …'` 처럼 bash 안에서 분리하거나, zsh 라면 `${=TARGETS}` 를 쓴다.
